...
スクリプトを指定しない場合、自動的にREPLが起動します。インタプリタ
REPLとJIT REPLのどちらも入力間でセッション状態を保持します — 一つの
入力で宣言した`let` / `mut` / `fn`は、ネストしたクロージャからも含め、
後続の入力から参照できます。JIT REPLはセッション中`LLJIT`インスタンス
を1つ維持し、各入力を新規モジュールとして追加します。トップレベル
束縛はセッション単位の辞書に格納され、runtime helperがスレッドローカル
ポインタ経由で読みに行くので、プロンプトで定義したクロージャは呼び出し
時点でトップレベル名を解決します。

REPLで定義されたクロージャ内の自由変数参照は、アクセスごとに1回の
ハッシュ検索を伴います（cellキャプチャの代わりにbody内で`repl_get`
を発行）。関数リテラル内（プロンプトではなく）で宣言されたbodyは
