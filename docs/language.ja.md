...
* **エフェクトフル**な `effect fn` は `handle` ブロック内から（直接、または
  別のエフェクトフル関数経由で）呼び出す必要があります。駆動されない通常の
  コードから呼ぶと `EffectError` を送出します — ハンドラなしには実行できません。
...
* エフェクトとジェネレータは**合成できます**: `effect fn` / `handle` 本体で
  ...
  （yield 式の中やループの中でも）。残る境界は parse 時に（対称に）拒否
  されます: エフェクト本体の**裸の `yield`**（本体自体はジェネレータでは
  ありません — ネストしたジェネレータ関数に包んでください）、ジェネレータ
  本体で `handle` の外にある **`perform`**（ジェネレータの中断はエフェクト
  境界を越えられません）、ジェネレータ本体の中の **`effect fn` 宣言**。
