...
`&`（and）、`^`（xor）、`<<` / `>>`（左 / 算術右シフト）、単項 `~`
（補数）は **2 つの `Long`**（`~` は 1 つ）に作用します。非 `Long` は
`type error`。`>>` は符号を保つ算術シフト。シフトも他の `Long` 演算
同様にラップします（bignum なし）。
...
優先順位（Python 準拠）: 比較 `<` xor `^` `<` and `&` `<` シフト `<` 加算。
よって `1 << 2 + 1 == 8`（`1 << (2+1)`）、`2 & 3 ^ 1 == 3`
（`(2 & 3) ^ 1`）。`~` は単項レベル。

**bit-OR `|` は未対応**: 単一 `|` 中置は `|...|` lambda 区切りと
stateless 文法下で衝突するため。disjoint なフラグビットは `+` で
合成する（`READ + WRITE`）。`|` 形は follow-up 予定。
