...
入口の検査は値が名乗るクラスを見ますが、普通のObjectは好きなクラス名を
名乗れます（`{class: 'Point', x: 'no'}`）。そこで仮定は使う場所で
検証されます: 書き込み検査を通っていないプロパティをそのfieldとして
読むと、fieldと型を名指す`TypeError`になり、違う種類の値として読まれる
ことはありません。本物のインスタンスがこれに出会うことはありません。
...
# Objectはどんなクラス名でも名乗れますが、そのプロパティは書き込み検査を
# 通っていないので、読みは違う種類の値を答える代わりに拒否します。
# !! TypeError
