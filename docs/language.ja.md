...
Culebraは小さな動的型付けスクリプト言語です。設計上の優先事項:
...
| `a.sort(reverse: Bool = false) -> Nil` *(破壊的)* | 自然順でin-place安定ソート。要素は`<`と同じ規則で比較し、Objectの`__lt__` / `cmp`を尊重する（`Path`配列もソート可）。比較不能な要素はthrow。キーワード専用`reverse: true`で降順（安定のまま） |
