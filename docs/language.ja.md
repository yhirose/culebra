...
* オブジェクトのプロパティ値に注釈は付けられません — クラス宣言の外で
  ...
  fieldだけが例外で、その注釈は書き込みのたびに検査されます（§13）。
* 配列要素の型は追跡されません。
...
クラスで注釈されたパラメータは入口で検査され、そのクラスの宣言field
は書き込みのたびに検査されます（§13）。したがってその名前を通じた
fieldの読みは、何が入っているかを既に知っています。コンパイラはこれを
使います: `p: Point`で`Point`が`x: Float`を宣言していれば、`p.x`は
`Float`の読みとしてコンパイルされ、値への問い合わせは行われません。
