...
drop されます。決定的なスコープ離脱の窓を外れるケースが2つあります:
**closure** に保持された循環（closure は定義スコープ全体を捕捉し、
スコープより長生きしうる escape として扱う）と、**トップレベル**
（離脱しないスコープ）で直接捨てられた循環ガベージです。これらは
...
発火します — スコープ離脱時、循環メンバーを含む。closure 保持の循環と
トップレベルの孤児は GC バックストップへ繰り延べ（上記。トップレベルの
*束縛*はプログラム終了時に drop されずに leak したまま）。well-known
