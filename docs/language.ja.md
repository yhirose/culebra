...
解釈と `{...}` 補間の両方を無効化するので、`\d`・`\w`・`{n}` 量化子は
そのまま通り、閉じクォートだけが区切りになります。`"..."` がここでは
補間しないのはこのためで、regex の `{4}` 量化子が `{expr}` 補間と衝突
するのを避けています。**動的な**パターンは文字列を組み立てて
`Regex.compile(...)` に渡してください（リテラル内に補間はなく、regex
インジェクションも防げます）:
