...
`self` はレシーバを伴う呼出でのみ存在します。レシーバなしで到達した
本体 —— 素の `f(x)`、UFCS 呼出、`map` などの組込みに渡した関数 ——
の中で `self` を参照すると、他の未束縛の名前と同じく `NameError:
undefined variable 'self'` になります。送出されるのは本体に入った
時点ではなく、`self` を読んだその箇所です。
