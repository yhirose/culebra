...
JIT メモ: 本マイルストーンは kwargs をインタプリタで完全に配線
しています。JIT はユーザ定義関数への kwarg/splat 呼び出しを
コンパイル時に拒否します（ビルトイン（`JSON.stringify` 等）は
自前で kwargs を処理します）。ユーザ関数に対して kwargs を使いたい
場合は `--jit` を外して実行してください。
