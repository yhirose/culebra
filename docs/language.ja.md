...
* `NAME = EXPRESSION` / `NAME: Type` / `NAME: Type = EXPRESSION`は
  **インスタンスfield** を宣言します — `new`本体の前に初期化される
  ...
  宣言型は**書き込みのたびに検査されます**。パラメータや`let x: T`と
  同じruntime-checkモデル（§14）で、`Float`・`Long`・`Bool`は自分の値
  だけを受け取り、それ以外（クラス、union、注釈なし）は従来どおりです。
  検査は値がfieldに届く経路すべてを覆います: リテラルの名前、計算した
  キー、コンストラクタ自身の`self.x`、nativeのbuilder。これにより
  宣言型を読む価値が生まれます — `Float`のfieldは`Float`を持つので、
  クラスが分かるコードは問い合わせずにfieldの型を知ることができます。
  `@packable`クラス（§21）はこれに加えて固定バイトレイアウトの計算に
  型を読み、`@value`クラスは全fieldがスカラかの検査に型を読むため、
  どちらもfieldはtyped必須です（どちらのクラス内でも`x = 7`は
  SyntaxError）。
...
* オブジェクトのプロパティ値に注釈は付けられません — クラス宣言の外で
  オブジェクトが生やしたプロパティに制約はありません。クラスの*宣言*
...
される場所で検査され、そのクラスの宣言fieldは書き込みのたびに検査され
ます（§10）。したがってその名前を通じたfieldの読みは、何が入っているかを
既に知っています。コンパイラはこれを使います: `p: Point`で`Point`が
`x: Float`を宣言していれば、`p.x`は`Float`の読みとしてコンパイルされ、
値への問い合わせは行われません。`mut`の束縛はこの約束を受け取りません —
再代入は再チェックされないので、宣言を読み取る根拠がありません。
...
同じ理由で`remove`は宣言fieldを拒否します — 消えうるfieldは契約に
なりません。
