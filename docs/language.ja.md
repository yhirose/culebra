...
* `NAME = EXPRESSION` / `NAME: Type` / `NAME: Type = EXPRESSION`は
  ...
  **書き込みのたびに検査されます**。パラメータや`let x: T`と同じ
  ...
  検査は値がfieldに届く経路すべてを覆います: リテラルの名前、計算した
  キー、コンストラクタ自身の`self.x`、nativeのbuilder。これによりそう
  したfieldの型を読む価値が生まれます — `Float`のfieldは`Float`を持つ
  ので、クラスが分かるコードは問い合わせずにfieldの型を知ることが
  できます。
