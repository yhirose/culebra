...
* エフェクト本体内のエラーは**元ソースの行**で報告されます。列は近似です
  （lower がずらすため）。
* エフェクトとジェネレータは今のところ 1 つの本体の中では合成できません:
  `yield` は `effect fn` / `handle` 本体の中に書けず、エフェクト構文
  （`handle` / `perform` / `effect fn`）はジェネレータ本体の中に書けません。
  どちらも parse 時に（対称に）拒否されます。片方をもう片方の外で実行し、
  値を渡してください。
