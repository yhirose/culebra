...
drop されます。保守的に残る例外が2つあり、メモリはサイクルコレクタが
回収しますが `drop` は走りません: **closure** に保持された循環
（closure は定義スコープ全体を捕捉し、スコープより長生きしうる
escape として扱う）と、**トップレベル**（離脱しないスコープ）で直接
捨てられた循環ガベージです。これらは最後の参照が解放される前に循環を
手動で切る（`a.other = nil` など）、明示的に `.drop()` を呼ぶ、
または所有元のスコープで `defer` (§15) を使ってください。
...
発火します — スコープ離脱時、循環メンバーを含む。ただし closure
保持の循環とトップレベル束縛（上記）には呼ばれません。well-known
...
* **Auto-drop (§16).** 参照カウントが 0 になったタイミング（スコープ
  ...
  スコープ離脱時に drop（closure 保持・トップレベルの循環は除く、
  §16 参照）。
