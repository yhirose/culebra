...
executorとそのLLVM lowering（`--jit`とAOT）— は本書の記述どおりに
実装することを要求され、エンジン間または本書との差異はバグです。

実例つき入門は [`handbook.ja.md`](handbook.ja.md)、標準ライブラリの
...
23. [既知の制約](#23-既知の制約)
24. [モジュール](#24-モジュール)
25. [付録: VM ↔ JIT の差分](#25-付録-vm--jit-の差分)
...
* **機能の直交性。** 新しい機能は文法の特別扱いとしてではなく、
  既存の機能と組み合わさる形で入ります。
* **1つのフロントエンドと2つのエンジン。** バイトコードVMとLLVM
  ORC JITは同じパーサ・AST・バイトコードコンパイラを共有する2つの
  消費者であるため、新機能は両方で実装できる必要があります。
* **予測可能なメモリ管理。** 両エンジンで参照カウントを採用。
  循環参照はサイクルコレクタが回収します。
...
CRLF(`\r\n`)で保存されたファイルも受け付ける。パースの前にLFへ正規化するので、
CRLFのcheckoutはLFのものと完全に同じに振る舞う — 三重引用符文字列やraw文字列の
中も含めて（そうしないと`\r`が値の一部になる）。`culebra fmt`はLFで書き出す。

単独の`\r`（後ろに`\n`が続かないもの）は3つめの改行ではなく構文エラー。
旧Macの改行なのか紛れ込んだバイトなのか決まらず、どちらかでプログラムの意味が変わる。

### コメント

* 行コメント: `# ...`または`// ...`、行末またはファイルの終わりまで。
* ブロックコメント: `/* ... */`、ネスト不可。
...
`let with = 1`も`if handle { … }`も変数を指します。**ループのラベル**
（`outer: for … { break outer }`、§12）も同じく文脈依存ですが、語を
1つも予約しません。`while`／`for`の直前と`break`／`continue`の直後で
しか認識されないので、使った名前は変数名としてそのまま空いています。
型注釈の型名
...
`Long`と`Float`の間では、算術時に自動的に`Float`へ昇格します
（§7参照）。比較はこの昇格を通さず値どうしを厳密に比べます — 昇格すると
2^53を超えたところで別々の`Long`が1つの`Float`に重なるためです。
これ以外の型間の暗黙変換はなく、算術・比較・論理演算子が
期待しない型を受け取ると`type error`となります（§15参照）。

`Any`は型注釈でのみ有効で、任意の値に一致します。

**両エンジン対応**：`Float`はバイトコードVMと
LLVM ORC JITのどちらでも完全にサポートされています。Longのみの
コードパスは`--jit`下でも従来どおりインライン整数コード生成を
維持し、Floatが混ざった場合のみLong↔Float昇格のスローパスに
入る設計で、ビット単位で同一の意味論を保証します。
...
* `Array`, `Object`, `Tuple`, `Set`: **値（構造的）等価**（要素を
  ...
  それが決めます — `a == b`なら`[a] == [b]`です。ただし**キーとして
  照合されるもの**（`Set`のメンバ、`Object`の`String`以外のキー）は
  検索時と同じ照合、つまり`hash`と`eq`によります。この段は`__eq__`を
  読まず、型もまたがないので、`[1, 7] == [1.0, 7]`が真でも
  `{1, 7} == {1.0, 7}`は偽です。`Function`と`Tensor`のみ参照同一性。
...
* `let`/`mut`がない裸の`x = v`はスコープ連鎖を検索します:
  * どこかのスコープ（外側のクロージャやグローバルを含む）に`x`が
    ...
    が状態を書き換える仕組み**です。
  * 見つからなければ、いちばん内側のスコープに新しい（イミュータブルな）
    束縛を作成します。代入を書いたブロック、ループ本体、matchの腕がそれに
    あたります。`if`の腕は外側とスコープを共有するので、そこで作った束縛は
    `if`の後でも見えます。

### 複合代入

12種類の複合代入演算子があります。いずれも`let`/`mut`と組み合わ
...
* **キャプチャされた状態 = オブジェクトの状態**。クロージャベース
  のオブジェクトパターン（[`handbook.ja.md` §9.2](handbook.ja.md#92-クロージャベースの別解) 参照）では、外側関数
...
spec自身も`{expr}`フィールドを持てるので、幅や精度をソースに直書き
せず計算できます:
...
フィールドは普通の式で、書かれた位置で（左から右へ、整形対象の値の
後に）評価されます。値は`Long`でなければなりません — 10進表記として
specに差し込まれるものであり、それが幅・精度の正体だからです。
他の型はフィールド自身の位置で`TypeError`になります。フィールドは
同じ桁を手で書いたのと**厳密に同じ意味**になるので、幅`0`は「幅なし」
ではなくゼロ埋めフラグとして読まれます: 数値なら問題なし
（`"{n:>{0}}"`）、文字列なら`ValueError`で、これは`"{s:>0}"`が元から
そうであるのと同じです。補間が使えるところならどこでも使えます:
...
スライスは境界で例外を投げません。端はインデックスなので`Long`で
なければならず、`Float`の端を持つrangeでスライスすると`TypeError`です。
...
後で添字に使えます（`xs[r]`）。両端が`Long`の境界付きrangeは反復可能
（`for i in 1..4`）。境界の無いrange（`2..`）は反復終端が無く、反復すると例外。
...
その値を作ったクラスの名前を先頭に付けます。`x`, `y`フィールドを持つ
`class`構文のインスタンスは`Point {mut x: 3, mut y: 4}`と表示されます。
普通のObjectはクラスを持たないので`{...}`と表示され、`class`という名前の
フィールドがあってもそれは普通のプロパティとして一覧に並びます。
...
宣言フィールド（宣言順）→ `self.x = y`で設定されたフィールド、の順に
なります。クラス名もメソッドも自身のエントリではなく、全インスタンスが
委譲する1つのクラス毎metaの上にある（§25参照）ため、どちらも列挙には
現れません。したがってインスタンスのスプレッドはフィールドだけを普通の
Objectに写し、その結果は何のインスタンスでもありません:
...
この書き方で作った Object は辞書であり、辞書として振る舞います。
キーの型を問わず、挿入・読み出し・削除は要素数が増えても O(1) の
ままです。Object リテラルやクラスインスタンスが持つ固定スロット
配置は別の表現で、キーが増えてその配置が割に合わなくなると
Object はそこから離れます。表現が変わるだけで、振る舞いは
変わりません。
...
2. 存在しない場合、外側スコープに`name`という名前の自由関数が
   あり、**その第一引数が`receiver`を受け取れる**なら、`receiver`を
   第一引数として`name(receiver, args)`を呼ぶ。これが
   **Uniform Function Call Syntax (UFCS)** で、D / Nimの流儀に
   倣っています。後述の[第一引数が決める](#第一引数が決める)を参照。
...
います —— 自前の`drop`を持たないレシーバは、それを受け取れる
スコープ内の自由関数`drop`に渡り、名前を自分で解決するレシーバ
（自前の`drop`を持つハンドル、あるいはそういう候補がそもそも無い
場合）だけがat-most-onceガードに届きます。クラスオブジェクト・enum・名前空間は決して届きません:
`C.drop()`はstaticな`drop`の普通の呼び出しです。

#### 第一引数が決める

自由関数がそのレシーバのUFCS候補になるのは、第一引数がレシーバを
受け取れるときだけです。型注釈の無い第一引数は何でも受け取ります。
注釈があれば、普通の引数として渡したときに通るもの (§14) を
受け取ります —— union、レシーバのクラスが適合するtrait、3段階の
どのレベルのenumでも同じです。先頭に位置引数を持たない関数
（引数なし、または`*args`だけ）は、失敗しうる宣言が無いので候補の
ままです。multimethodでは、第一引数がレシーバを受け取れるoverloadが
1つあれば足ります。どのoverloadが走るかは、その後の全引数に対する
通常のディスパッチ (§20) が決めます。

レシーバを受け取れない関数は、その呼出にとっては存在しないのと
同じです。だからメソッド名の書き間違いが、無関係な関数の引数に
ついての苦情に化けることはありません:
...
この判定はレシーバだけを見て、どの引数よりも先に行われます。残りの
引数はこれまでどおり、呼ばれた側のバインダが検査します。
...
`x.m(a, b)`はプロパティを読み、**すべての**引数を左から右に評価し、
その後でバインドします。したがって引数の型違反も、レシーバが持って
いないメソッドも、リスト全体が走り終えた*後で*報告されます:
...
`(5).push(loud())`はプロパティ読みの時点で失敗し、`loud()`は走り
ません。
...
なければ自由関数として解決し、その第一引数がレシーバを受け取れる
ならレシーバを第一引数として呼び出します。
...
代替として`class`形式があり、同じランタイム表現に脱糖されます:
    ...
    inspect(type_of(c))         # 'Car' — それを作ったクラス

セマンティクス:

* 宣言は`Car`を`new`プロパティのみを持つ`Object`に束縛します。
* `Car.new(...)`はコンストラクタが設定したフィールドを持つ新しい
  `Object`を返します。コンストラクタ本体の実行中は`self`がその新
  オブジェクトに束縛されます。クラス名もメソッドもインスタンスには
  コピーされません — どちらも全インスタンスが委譲する1つのクラス毎
  metaの上にあり、`c.run`はそこを経由して解決され、`type_of(c)`は
  そこから名前を読み、`c.keys()`はフィールドのみを返します
  （*列挙*参照）。Objectはフィールドを書くことでクラスを名乗ることは
  できません — `type_of`・`match`・クラス型の引数・`==`はどれもmetaに
  問い合わせます。
* クラスを呼ぶとコンストラクタが呼ばれます: `Car(5)`は`Car.new(5)`で、
  キーワード引数もそのまま渡ります。この規則は値の書かれ方ではなく
  値が持つ`new`に対して定義されているので、ライブラリ側の
  コンストラクタにも同じく及びます — `Vector2(1, 2)`、
  `Scene.Image(4, 4)`、`Channel()` — docsに載っている`.new(...)`で
  この形が使えないものはありません。及ばないのは`new`キーを持つ
  ただの`Object`で、こちらは呼び出し不可のままです。fieldに名前を
  付けただけでdictを呼び出し可能にはできません。
* コンストラクタ/メソッド内で`self.x = y`で作られるフィールドは
  **デフォルトで可変**です（通常の`o.x = y`は不変プロパティを作る
  のと異なります）。メソッドが頻繁にインスタンス状態を書き換える
  クラスの慣習に合わせています。
* `new`メソッドは省略可能で、省略時は無引数・フィールドを1つも
  持たないインスタンスを返します。
* コンストラクタ本体内の`self`は不変です。`self = newObj`への
  ...
  として書いてください。
* メソッドの引数と戻り値には`fn`と同じ型注釈を書け、検査のされ方も
  同じです（*戻り値*を参照）。`area() -> Float { ... }`は本体の末尾で
  抜けるときも`return`で抜けるときも戻り値を検査し、
  `c.area.return_type`は`'Float'`を返します。`static`と`get`の
  メンバにも書けます。例外は`new`で、結果は常にインスタンスなので
  `new(...) -> T`は`SyntaxError`です。
...
* `NAME = EXPRESSION` / `NAME: Type` / `NAME: Type = EXPRESSION`は
  **インスタンスfield** を宣言します — `new`本体の前に初期化される
  ...
  - **引数バインド後・`new`本体の前**: 初期化子はバインド完了後に
    ...
    ではありません）。ctor引数をfieldに入れるには本体で`self.x = a`と
    明示するか、field parameter（`new(.x)`、下記）を使います。
  ...
  **スカラ**の宣言型（`Float`・`Long`・`Bool`、および§21の固定幅の綴り
  `Float32`・`Int32`・`Byte`…。いずれもこの3つのどれかを表します）は
  **書き込みのたびに検査されます**。パラメータや`let x: T`と同じ
  runtime-checkモデル（§14）で、そのfieldは自分の値だけを受け取ります。
  検査は値がfieldに届く経路すべてを覆います: 宣言自身の初期化子、
  リテラルの名前、計算したキー、コンストラクタ自身の`self.x`、
  nativeのbuilder。これによりそうしたfieldの型を読む価値が生まれます
  — `Float`のfieldは`Float`を持つので、クラスが分かるコードは
  問い合わせずにfieldの型を知ることができます。

  **それ以外の注釈に書き込み検査はありません。** 上のゼロ値の選択には
  従来どおり効き、コードの意図も記しますが、fieldは渡されたものを受け
  取ります: `String`、クラス、union、オプショナル（`T?`）、注釈なし。
  これは`@value`が自分のfield型を絞るときと同じ理由づけです（§21） —
  スカラとは、tagだけで型が決まり、背後にヒープ本体を持たず、同じものの
  第二の綴りが存在しない値のことです。`String`には第二の綴りがあります:
  `StringView`（§18.1）も文字列値で、`slice`・`split`・`lines`・文字列に
  対する`for`が返すのはこちらです。したがって`name: String`のfieldは
  `StringView`を受け取り、その読みは従来どおり値に型を問い合わせます。

  `@packable`クラス（§21）はこれに加えて、fieldの宣言型を固定バイト
  レイアウトの計算に読み、`@value`クラスは全fieldがスカラまたは別の
  `@value`クラスかの検査に読むため、どちらもfieldはtyped必須です
  （どちらのクラス内でも`x = 7`はSyntaxError）。
* `new(.x)`は **field parameter** を宣言します: `new`のパラメータ`x`
  であり、同時にその引数をfield `x`に格納するので、本体にその代入を
  書きません。`.x: T`と`.x = v`は他のパラメータと同じ形で型と
  デフォルトを与えます。`.x?`は省略可能にし、型とデフォルトの両方を
  fieldの宣言から取ります:
      ...
      inspect(c.has_battery)     # false（宣言のデフォルト）
      inspect(c.chr_ram.size())  # 8192（初期化子がchr_romを読んだ）

  - `x`という名前の普通のパラメータです: キーワードでは`x`として渡し、
    arity/型のエラーでも`x`と報告され、後続のデフォルトや本体からも
    `x`として読めます。位置/キーワード渡し、`*`、並び順の規則（必須
    パラメータは省略可能なものの後に置けない）はそのまま適用され、
    `.x?`は省略可能側に数えます。
  - 引数は宣言順のそのfieldの位置で、引数のバインド後・`new`本体の前に
    格納されます。したがってそのfieldより*下で* 宣言された初期化子は
    引数が格納した値を読め（上の`chr_ram`）、*上で* 宣言されたものは
    従来どおり`nil`を読みます。格納は他のすべての格納と同じ検査付きの
    書き込みで、スカラの宣言型は別の型の値を拒みます。
  - 呼び出し側が`.x?`を省略すると、宣言自身の値（初期化子、または型の
    ゼロ値）が入り、本体のパラメータ`x`もその値として読めます。`nil`を
    渡すのは値を渡すことであって、省略ではありません。
  - 宣言済みのfieldを名指す場合: パラメータが`.x?`でない限り、宣言は
    初期化子を持てません（さもないとパラメータが常に値を供給し、
    初期化子が走ることがない）。パラメータ側の型は宣言の型と一致
    しなければなりません。`.x?`は自分の型もデフォルトも持ちません。
  - 宣言されていないfieldを名指す場合: `.x`と`.x: T`はそのfieldを
    **宣言** します。クラスのfield一覧では最初の`new`の位置に入り
    （各オーバーロードが宣言するfieldはすべてそこに、名指した順で
    並びます）、型は書いたとおり（型なしなら`nil`で始まり、ゼロ値も
    検査もありません）。複数の`new`オーバーロードが同じfieldをこの形で
    宣言してもよく、型の書き方は一致が必要です（型なしの`.x`も含めて）。
    名前は本体の宣言と同じ一意性の規則に従います。`@value`や
    `@packable`クラスではそのfieldもfield規則に従います（型が必須）。
    `.x?`にはデフォルトを取る宣言がないのでSyntaxErrorです。
  - この形を受け付けるのは`new`のパラメータリストだけで、他の場所では
    SyntaxErrorです。
* `get NAME () { ... }`は **getter**（引数なしメソッド）を宣言します。
  ...
  `obj.name`と`obj.name()`の両表記はVM・JIT・AOTで同一に発火します。
* 両エンジンともクラス宣言をコンパイルします。
...
| `a == b`      | `__eq__`     | `!=`は否定から導出                    |
| `a < b`       | `__lt__`     | `>=`は否定から導出                    |
| `a <= b`      | `__le__`     | 未定義で`__lt__`があれば`a < b or a == b`として導出。`>`は否定から導出 |
...
traitのメソッドにフォールバックします。`==` / `!=`はクラスが書いた
`eq(other)`を、`<` / `<=` / `>` / `>=`は`cmp(other)`（`Comparable`の
正準メソッド）を経由します。これにより手書きの`eq` / `cmp`クラスが、
`__eq__` / `__lt__`を書かずに`a == b` / `a < b`をそのまま使えます。
優先順位は **明示dunder > traitメソッド > 既定**（`==`は構造比較、
順序比較はどちらも無い`Object`で`type error`）。書かれた`eq`を経由する
ことで、演算子は同じ`eq`を使う`Object` / `Set`のキー等価と一致します。
**派生した**`eq`は書かれたものではなく「インスタンスはfieldで一致する」
と言っているだけなので、そのclassの`==`は構造比較 — 既定 — になり、
enumのvariantと同じ扱いです（*methodの自動導出*を参照）。
...
異なる場合に有用です。両エンジンは同じウォーカーで合成しており、
出力順序も一致します。
...
パターンの照合はどの対象に書き込むより前に終わるので、形の不一致
（`ValueError`）では1つも書き込まれない — `[a, 5] = [9, 6]`は`a`に`9`を
入れてから`5`で失敗するのではなく、`a`を変えずに終わる。
`ImmutableError`はこれとは別で、その対象への書き込み時に送出される
ため、左側の対象は既に新しい値を持つ。
...
* パラメータは`name = expr`の形でデフォルト値を持てます。呼び出し側
  ...
  定義環境に「パラメータより先にフレームが作る束縛」——レシーバ
  `self`、再帰ハンドル`fn`、先行パラメータ——を重ねた環境で評価され
  ます。したがって`fn (a, b = a + 1)`も`m(k = self.n)`も書けます。
  コンストラクタではインスタンスは既に存在しますがフィールド初期化子
  は未実行なので、そこでの`self`のフィールドは`nil`です。
...
**本体の中では**、`while`・`for`・`if`・`{ }`の本体の任意の深さに
`yield`を置け、`break` / `continue` / `return`は通常の関数と同じように
働きます（`return`は生成を終了します）。`defer`は通常の関数と同じく、
そのスコープを抜けるときに実行されます (§15)。ループ本体の`defer`は
反復ごとに、本体直下の`defer`は本体が終わったときに走ります。本体の中の
`for`-inも同じ規則で反復子を閉じます。
...
ときです。その時点で残っている`defer`が内側から順に実行されます。後始末は
本体が最後まで到達したかどうかに左右されません。
...
ジェネレータは全レーンが共有するソースレベル変換で
コンパイルされるため、同一のプログラムはVM・JIT・AOT
バイナリのいずれでも同じ値を生成します。
...
rangeの値`a..b`（排他）と`a..=b`（包含）は`range`と同じ遅延整数列を反復します。
両端が`Long`で揃ったrangeは反復でき、スライス用の開いたrange（`xs[2..]`）は
反復の終わりがないので、反復すると例外になります。

端点には`Float`も使えます。その場合rangeは列ではなく数値の区間で、作る・表示する・
比較することはできますが、反復・スライス・`grid`への引き渡しはその場で
`TypeError`（`expected Long, got Float`）になります。数値でない端点は、rangeを
作った時点で`TypeError`です。
...
**所属判定** — `r.contains(x)`は区間として判定します: `start <= x`かつ
`x < end`（`..=`なら`x <= end`）で、開いた端は何も制限しません。`Long`と
`Float`は`<`と同じように比べます。数値でない引数と`NaN`はどのrangeにも
含まれず、引数が原因で例外になることはありません。`by`の刻み幅が1以外の
rangeは`ValueError`です。
...
はiteratorプロトコルを実行時に駆動します — Array/String/keysの
高速パスはそのまま維持。
...
**ラベル。** `while`と`for`にはラベルを付けられ、`break` / `continue`
はそのラベルを指定できます。指定すると、最内ループではなく**その
ループ**を抜ける（あるいは次のiterationへ進める）ことになります:

    label: while cond { body }
    label: for var in iterable { body }

    break label       # `label`のループを抜ける
    continue label    # `label`のループの次のiterationへ
...
ラベルは`break` / `continue`と同じ行に置く必要があります。そのため、
ラベルなしの`break`の次の行が識別子で始まっていても、2つの文のまま
です。ラベルが届くのは、それが書かれた関数の中のループだけです —
入れ子の`fn`や`defer`ブロックの中の`break outer`は、そこでの
ラベルなしの`break`と同じく`SyntaxError`になります。囲むループの
どれも持たないラベルを指定した場合と、囲むループが既に持っている
ラベルを付け直した場合も、実行前に検査されて`SyntaxError`です。

ラベルは束縛ではありません。同じ名前は変数名としてそのまま使えますし、
並んだ別の入れ子で同じラベルを使い回せます。

ラベルで入れ子を抜けるのは、1つずつ抜けるのと同じ抜け方です — 途中の
ループはそれぞれの`defer`ブロックを（内側から）実行し、イテレータを
閉じてから、ジャンプ先に着きます。
...
ループ**に属します（ラベル付きでも同じで、他の場所と同じように入れ子を
抜けます）。逆に、本体からのラベル付き`break`で抜けられたループは、
自身の`nobreak`ブロックを実行しません — breakで抜けられたからです。
`nobreak`は文脈依存キーワードで、この末尾位置でのみ認識されるので、
他の場所では通常の識別子として使えます。
...
| `-1`, `-2.5`     | 負の数値リテラル。`1`と同じく型まで一致する等価 |
| `lo..hi`, `lo..=hi`, `..hi`, `lo..` | 区間に入る`Long`または`Float`（`Range#contains`と同じ判定）。端点は数値リテラル |
| `name`           | 任意の値、`name`に束縛                      |
| `_`              | 任意の値、束縛なし                          |
...
* リテラルパターンはコンパイル時定数でなければなりません。**補間入り**
  ...
  （`s if s == x => ...`）。
* 範囲パターンは、`(lo..hi).contains(x)`が受け付ける数値に一致します。
  `Long`と`Float`は`<`と同じように比べるので、リテラルパターンと違って
  対象がどちらの型かは問いません: `match 5.5 { 0..10 => … }`は一致し、
  `match 1.0 { 1 => … }`は一致しません。数値でない対象と`NaN`は例外に
  ならず次のアームへ進みます。端点は数値リテラル（負号可）で、`by`は
  書けません。`5.. =>`は空白を空けて書きます（`5..=>`は`..=`と読まれます）。
...
* 算術・論理演算子は自身のオペランド型を検査しますが、中間値の
  注釈は追加の検査にはなりません。
* オブジェクトのプロパティ値に注釈は付けられません — クラス宣言の外で
  オブジェクトが生やしたプロパティに制約はありません。クラスの*スカラ*
  型（`Float`・`Long`・`Bool`、またはその固定幅の綴り）で宣言された
  fieldだけが例外で、その注釈は書き込みのたびに検査されます（§10）。
  `String`を含むそれ以外のfield注釈は検査されません。
* 配列要素の型は追跡されません。
* `mut x: T`の注釈、および`mut`パラメータの注釈は、宣言時または呼び出し
  時の一度のみの検査で、後続の再代入では再チェックしません。

注釈は主に**ドキュメントと境界検査**のための機能であり、型システム
ではありません。

### プリミティブ型のパラメータが確定させること

プリミティブ型名（`Long`・`Float`・`Bool`・`String`・`Array`・`Object`・
`Set`・`Tuple`）で注釈されたパラメータは入口で検査されるので、本体での
その名前の読みは既に注釈を満たしています。コンパイラはこれを前提として
扱います: そうしたパラメータへの組み込みメソッド呼び出しは、その名前を
共有する複数の型のどれを持っているかを調べる代わりに、型が名指す1つの
受け手に解決されます。`Long`や`Float`のパラメータの算術は、オペランドの
検査を落とします。答えはどちらでも同一で、注釈は問い合わせを省くだけです。

2つの注釈は何も確定させません。`mut`パラメータは再代入でき、それは
再チェックされません（上記）。`Function`は構造的に満たされ（`__call__`を
持つクラスのインスタンスが通る）、1種類の値を名指しません。クラス名は
クラスを確定させますが表現は確定させません。それが読み手に何を許すかは
§10が扱います。
...
# 確定させているのは入口の検査であり、それ以外は従来どおり拒否されます。
# 型付きパラメータは多重定義のシグネチャなので、拒否はDispatchErrorです。
# !! DispatchError
width(42)
```

```culebra
# `mut`パラメータは後続の読みについて何も約束しません: 再代入は
# 再チェックされないので、本体は渡されたものをそのまま見ます
...
宣言クラスの分かる名前 — クラスを名指す注釈を持つパラメータか`let`、
そのクラス自身のメンバ内の`self`、クラス型fieldを辿った次の段 — は束縛
される場所で検査され、そのクラスのスカラ宣言fieldは書き込みのたびに検査
されます（§10）。したがってその名前を通じた*そうした*fieldの読みは、何が
入っているかを既に知っています。コンパイラはこれを使います: `p: Point`で
`Point`が`x: Float`を宣言していれば、`p.x`は`Float`の読みとしてコンパイル
され、値への問い合わせは行われません。それ以外の型で宣言されたfieldは、
何が入ったかを誰も検査していないので、従来どおりの読みになります。`mut`の
束縛もこの約束を受け取りません — 再代入は再チェックされないので、宣言を
読み取る根拠がありません。

入口の検査が見るのは、値が何によって作られたかであって、どう見えるかでは
ありません。クラスの名前はインスタンスがたどるmetaにあり、普通のObjectは
fieldを持つことでそれを手に入れることはできません。ですからクラス名を
名乗るObjectは、fieldを読む前に、渡された場所で拒否されます。
読み側にも検査は残っています — 入口の検査を通っていない受け手のためで、
他のオブジェクトに移されたメソッド値が`self.x`を読む場合がそれです。
そこではfieldと型を名指し、違う種類の値を答えることはありません。
同じ理由で`remove`はスカラ宣言fieldを拒否します — 消えうるfieldは契約に
なりません。それ以外の型で宣言されたfieldは、何も約束されていないので
削除できます。
...
# クラス名を名乗るだけのObjectはそのクラスではないので、引数に束縛されません。
# !! DispatchError
...
* **Nominal**: 派生`eq`は同じclass tagを要求するので、同一field
  を持つ別class同士は等しくならない。
* **派生`eq`は等価を宣言しない**。「インスタンスはfieldで一致する」と
  言うだけで、enumのvariantと同じ扱いになる。キーとしては型が厳密で
  （`1`と`1.0`は別のキー）、だから`eq`は`hash`と一致でき、classが
  キーになれる。`==`では構造比較 — fieldを`==`で比べるので`Long`と
  `Float`が値として混ざる — になり、`eq`を持たないclassとまったく同じ
  答えを返す。キーにするために`Eq`を導出しても`==`は狭まらない。2つが
  食い違うのはこの混在した対だけで、`P(1) == P(1.0)`は真だが`P(1)`と
  `P(1.0)`は別のキーのまま。classが自分で書いた`eq`や`__eq__`は宣言
...
* 関数全体の終了時に発火させたければ、関数本体の先頭に`defer`を
  書く（関数本体自体がブロック）
* トップレベルの`defer`はプログラム終了時に発火 — importされた
  モジュールのトップレベルの`defer`も同じで、エントリモジュールのものの
  後に走る
...
から外へ新しい例外で続き、外側の`try`で捕まえられます。`try`本体自身の
deferは、正常終了でも`throw`での終了でも、その`catch`の届かない所で
走ります — そこから投げられた例外を受け取るのは1つ外側の`try`です:
...
挙動はVM / JIT / AOTバイナリで同一です。`Interrupted`は
...
標準ライブラリの中で起きたエラー（Culebraで書かれたライブラリの関数や、
それが呼ぶネイティブ関数が投げたもの）は、そのライブラリを呼び出した
ユーザのコードの位置を報告します。`line`と`col`がライブラリのソースを
...
`e.col`をVM・JIT・AOT全backendで同一にpopulatedします
（注記がある場合を除く）。
...
`.backward()`も、評価前のグラフを破棄する操作も、どの深さでも
`ValueError`の上限なしに安全です。計算グラフには自然な「深すぎる」
という意味論が無く（長いsequenceに展開したRNNは正当なグラフであり、
壊れたデータではありません）、そのため人工的な深さ上限を設けず
内部的にboundされています（stdlibリファレンスのAutograd節参照）。
...
JITレーンは`throw` / `try` / `catch` / `defer`をVMと同一の
セマンティクスでサポートします:
...
渡した値でperform側のコードを再開します。引数なしの`resume()`は`nil`で再開します。
...
* 対象: `Array`, `Object`, `Tuple`, `Set`、およびクロージャが捕獲
  ...
  だけを経由する循環を含む）。
* `String`はrefcount管理されません。文字列バイトは`Runtime`ごとの
  slabから確保され、`String`は **traced-only** な値
  です — refcountが0になることはないので、上記の
...
`drop`が呼ばれ、さらに連鎖します。親 → 子の順序が保証され、
同一親の兄弟同士はプロパティ宣言順（Array/Tuple/Setの要素は
要素順）に解放されます——どのbackend・プラットフォームでも同じです。

**例外**: `drop`内から送出された例外はstderrに記録して握り潰され、
残りの解放カスケードが続行します。

**構築の失敗**: クラスインスタンスは`C.new(...)`に入った時点で——
引数が束縛される前、フィールド初期化子が走る前に——存在します。
したがってコンストラクタ呼び出しがどこでthrowしても（引数の不足・
型違い、デフォルト式のthrow、フィールド初期化子、本体）、作りかけの
インスタンスの`drop`は走ります。オーバーロードされた`new`の
ディスパッチ失敗はインスタンスを作る前にthrowするので、この場合は
何もdropされません。
...
**置き換えの順序**: スロットの中身を上書きするとき — `a[i] = v`、
`o.x = v`、`o[k] = v`、変数の再代入 — は、新しい値を先に格納し、
古い値をその後で解放します。したがって、その解放で走る`drop`から
そのスロットを見ると、dropされている値ではなく新しい値が入って
います。`drop`はそのコンテナを伸ばしても書き換えてもかまいません。
同じスロットに代入した場合は、`drop`自身の書き込みが残ります。
...
closure slotを通って循環するresourceも、所有スコープの離脱で
dropされます。どのレーンでも決定的な窓を外れる形が2つあります:
...
走らせます（PythonのPEP 442と同じ精神）。バックグラウンドの
コレクションはアロケーション数のしきい値で走り、根を保守的に
見つけるので、ある孤児に*いつ*到達するかは規定しません。
`GC.stat()`は参照カウントから根を決めるコレクションを走らせ、
その呼び出し時点で到達不能な孤児をすべてfinalizeします（全backend
共通）。同一コレクション内のfinalize順序は未規定です。**プログラム終了時**の
...
どのバックエンドもトップレベルスコープの解放時にdropを抑制し、
**未catchのエラー**による終了も同じ扱いです — エラーが通過した
スコープは内側から順に決定的に解放されます（各スコープの`defer`の
あとにそのスコープの束縛）が、トップレベル自身の束縛はdropされずに
残ります。スクリプト全体にわたるリソースは、どちらの終了経路でも
走る`defer`（§15）や明示的なクリーンアップを推奨します。

**JIT**: `--jit`でも自動dropはVMと同じタイミングで
発火します — スコープ離脱時、循環メンバーを含み、closure保持の形も
...
不問）はただの関数で、契約検査されず、自動呼び出しもされず、
`C.drop()`と書けば他のstaticと同じように呼ばれます。明示的な
`x.drop()`を1回だけに抑えるガードはインスタンスのものです。
クラスオブジェクト・enum・名前空間では`drop`も他と変わらないメンバー名で、
無ければ、存在しないメンバーを呼んだときと同じエラーになります。
...
走らせます。呼ばれるのはその反復自身の束縛が解放された後なので、ループ
変数と本体ローカルの`drop`はどのexitパスでも`dispose`より先に走ります。
すでに例外がunwind中のexitパスで`dispose`自身がthrowした場合は握り潰され、
外側の`catch`には元のエラーが渡ります。ループ変数のパターンが要素に
マッチしなかった場合もその例外にあたるので、`catch`に届くのはmismatchの
エラーです。
...
のようなArrayのeagerチェーン — が`--jit`でVMと
...
バリアントはenumを指しますが、生かしてはおかないので、ほかに誰もenumを
持っていなければ（戻り済みの関数の中で宣言し、コンストラクタを外に渡して
いないenumなど。enumから読み出したコンストラクタはenumを持ち続けます）、
そのバリアントは
`nil`を答えます。別の場所で作り直されたバリアント（`Channel`で受け取った
もの、`SharedBuffer`から読み戻したもの）も`nil`で、`Channel`で受け取った
classインスタンスと同じです。どちらの場合も、表示と比較は元のバリアントの
ままです。
...
`'Nil'`, `'Bool'`, `'Long'`, `'Float'`, `'String'`, `'StringView'`,
`'Array'`, `'Object'`, `'Function'`, `'Tensor'`, `'Tuple'`, `'Set'`,
`'Range'`のいずれか。あるいは、あるクラスが作った値ならそのクラス自身の
名前（enumのバリアントはバリアント名）。クラスオブジェクト自身は
`'Class'`です。つまりこの集合は開いていて、`type_of(v) == 'T'`は
`v: T`が引数に問うのと同じことを問います。違いは、注釈は部分型も受ける
（`Object`はどのインスタンスも受ける）のに対し、こちらは厳密な名前を
答えることです。
...
**最初に登録されたメソッドの**シグネチャを返します (それ以降の
オーバーロードは見えません)。
...
ある名前のオーバーロードは、それを宣言したスコープに属します。`if`や
`cond`のarmは独立したスコープではない（§6）ので、その中に書いた
`fn name`は、armが実行された時点から、外側のスコープのオーバーロードに
加わります:
...
上書きが及ぶのは宣言スコープの**1回の実行**の中だけです。関数本体や
ループ本体の中にある宣言は走るたびに新しいオーバーロード集合を作るので、
各本体はそれを宣言した実行時の捕獲を保ちます（先に得た`fn`値が後の実行の
本体に差し替わることはありません）。後述のクラス側のオーバーロード集合も
同じ規則です。
...
（`@derive(Eq, Hash)`が生成するのと同じ対）ので、フィールドで一致する
キーになり、`==`は構造比較になる。フィールドは宣言されたスカラーで
それぞれ型が1つなので、`1`と`1.0`を並べて持つ派生classのように2つが
分かれることはなく、`Set`の要素判定とObjectのキーは`==`と一致する。
通常のクラスが持たないのは`hash`のほうで、これが無ければインスタンスは
`Set`の要素にもObjectのキーにもなれない。自前の`__eq__`または`eq`を
書いたクラスはそれを保持し、対の両方を辞退する — これが対の整合を保つ。
...
| `--jit`        | バイトコードVMではなくLLVM ORC JITを使用               |
| `--jit-faststart` | `--jit`と同様だがIRと機械語の両方の最適化を省き、JITのウォームアップ時間を3〜5分の1にする。定常状態の代償はホットな処理のうち最適化器が手を出せる割合で決まる（C++/BLASランタイム側なら約0%、呼び出しの多いスクリプトコードで約30%、スカラー算術のタイトループでは数倍）。`-O0`を含意。出力は`--jit`と一致 |
| `--emit-llvm`  | `--jit`と併用で生成されたIRを出力して終了             |
| `-O0`〜`-O3`   | `--jit`の最適化レベル（デフォルトは`-O2`）            |
| `-h`, `--help` | オプションとコマンドの一覧を出力して終了                 |
| `--version`    | バージョン、開発ビルドならビルド元のコミット、対応バックエンドを出力して終了。リリースタグをクリーンなチェックアウトからビルドした場合はバージョンのみ |
...
VMのexecutorで動きます — プロンプトの1行がホットループになることは
...
は完全に等価です。どちらのエンジンも無条件に束縛するので、C++ホストに
埋め込んで動かすプログラムからも、スクリプトと同じ3つの名前が見えます。
...
完全に一致させ、依存の繋ぎ方も両エンジンで同じです: 全モジュールの
本体を1つのスコープ分離されたプログラムにコンパイルし、各依存
モジュール本体の後でコンパイル済みコードが
`culebra_runtime_module_register`を呼んで絶対パスとexport Objectを
登録。`IMPORT_STMT`は`culebra_runtime_module_get`にコンパイルされ、
同じパスで取り出して束縛します。
...
本書が規範的です。VMのexecutorとLLVM loweringは同じコンパイラが
生成した同じバイトコードを消費し、すべてのプログラムについて
**観測可能な挙動が一致する**ことを要求されます — 戻り値・副作用順・
エラー`kind` / `message` / 位置情報すべて同じ。内部表現は外部観測動作が
一致する限り自由に異なって良く、観測動作のずれはバグとして扱います。
リリース済みバイナリが、ゲートの比較対象となる独立した第二実装です。

### 観測可能な挙動が一致する点

VM・JIT・AOTすべてのビルドで同一が保証されます:
...
* **オブジェクトのプロパティストレージ.** 両エンジンがプロセス
  インターン化されたhidden-class（Shape）レイアウト + ベクトル
  バックのスロットを共有し、JITはコールサイト単位のinline cache
  を加えます。キー反復順は両者とも挿入順。
* **HOF inline展開.** `--jit`では`array.map / filter / for_each /
  reduce`や`range(N).<HOF>(...)` / `iter.map(λ).collect()`は
  lambda本体を反復ループ内に直接展開。副作用順は維持されます。
* **クラスメソッドの保管.** メソッドはクラス共有メタオブジェクト
  に置かれ、プロト委譲で参照されます。`obj.m`はどちらのエンジン
  でもバインド済み関数を返す。
* **サイクルコレクタの起動タイミング.** 両エンジンは世代別ではない
  フルのmark-sweepを1つ共有し、適応的な閾値（生存集合とlive bytes
  から再武装）で走らせます。つまりcollectの**タイミング**は実行間
  で違う。プログラムから観測できる挙動は同じで、循環メンバーも
  pre-sweepパスで`drop`が発火する（§17）。
* **Forward-ref pre-allocation.** 両エンジンとも各関数本体を
  スキャンして捕捉セルを事前確保し、`let`宣言より前にコンパイル
  されたclosureも宣言後の値を見られるようにします。
...
* **サイクルGCの内部方式.** 両エンジンは保守的なスタック走査
  mark-sweepを1つ共有します。あらゆる循環形状を回収します
  （`Object`のプロパティマップだけを経由する循環を含む）。
* **スレッド安全性.** ほとんどのランタイム状態（GC、defer stack、
  interrupt flag）は`thread_local`な`Runtime`に
...
エンジンでもプログラム終了まで`drop`を実行せず生存します（§17の
例参照）。エンジンはトップレベルスコープ解放時にdropを抑制します。
どのレーンで走らせるかに関わらず、スクリプト全体のリソースには
`defer`かfactory関数を使ってください。

疑わしい場合は本書が正。どのレーンであれ観測可能挙動のずれは
バグとして扱い、2つのレーンとリリース済みバイナリとの
differentialがそれを検出します。
