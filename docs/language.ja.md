...
| `--jit-faststart` | `--jit`と同様だがIRと機械語の両方の最適化を省き、JITのウォームアップ時間を約40分の1にする代わりに定常状態のスループットが小さく低下する（純スクリプトのホットループで約12%、ホット処理がC++/BLASランタイム側にある場合は約0%）。`-O0`を含意。出力は`--jit`と一致 |
