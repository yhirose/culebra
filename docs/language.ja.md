...
`Nil`, `Tuple`, ランタイム `String` — をサイドカーマップに書き込め
ます:
...
添字書き込みは shape ベースの `obj.foo` パスと統合**されません**。
`obj["foo"] = 1` （ランタイム String）はサイドカーに入り、`obj.foo`
（shape の高速パス）は依然として `nil` を返します。静的プロパティを
セットしたい場合は `obj.foo = 1` を直接使ってください。複合形式
（`obj[k] += v`）は Array のみ対応です。
...
* `match` アーム本体は単一式である必要がある（`{ ... }` は `Object`
  リテラルと解釈される）。
* ドット形式のプロパティ名は識別子のみ（`obj.foo`）。ランタイム
  String / Long / Float / Bool / Nil / Tuple をキーにする場合は
  添字パス（`obj[k]`）を使い、別のサイドカーマップに格納されます。
  shape ベースの `obj.foo` ネームスペースとは統合されません。
  ランタイム挙動は §10「添字代入」を参照。
