...
> `Encoding.url`, `Encoding.html` (`.encode`/`.decode`) are available
> (see stdlib §16). JSON is the top-level `JSON` namespace
> (`JSON.parse` / `JSON.stringify`) — there is no `Encoding.json`.
