...
`gc_refs`）から決めるので、どの参照カウントオブジェクトが生き残るか、
つまりどの孤児の`drop`が発火するかは、どのbackendでも毎回同じになる。
したがって`rc_objects`は決定的な値である。`live_objects`は保守的な
スタック走査でトレースされるStringも数えるので数個ぶれることがある。
2回の呼び出しの間に走るバックグラウンドのコレクションも保守的で、
より安価である。
