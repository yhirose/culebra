...
このコレクションは根をスタック走査ではなく参照カウント（CPythonの
`gc_refs`）から決めるので、プログラムのどのオブジェクトが生き残るか、
つまりどの孤児の`drop`が発火するかは参照カウントだけで決まり、どの
backendでも毎回同じになる。したがって`rc_objects`は同じプログラム・
同じエンジンなら決定的な値である（エンジンは自身の管理用オブジェクトを
少数持つので、2つのエンジンの値が互いに等しいとは限らない）。
`live_objects`は保守的なスタック走査でトレースされるStringも数えるので
数個ぶれることがある。2回の呼び出しの間に走るバックグラウンドの
コレクションも保守的で、より安価である。
...
結果だけです。backwardは閉形の式で勾配を求めます。CUDAでは3つのカーネルが
行ごとの統計量を計算し直して3つの勾配を直接書き、それ以外では融合前のopで
正規化した行を作り直します。どちらでも合成版と同じ勾配になります。
...
勾配を保持するのは葉だけです。中間ノードの勾配は、そのノードの入力へ
渡した時点で解放されるので、`.backward()`後のそのノードの`.grad()`はzerosを返します。
同じグラフに2回目の`.backward()`を通すと、葉にはその回の寄与だけが
加算されます。
...
演算だけをGPUに送ります。行列積をどの大きさからGPUに送るかは、
`auto`での最初の評価のときにそのマシン上で測って決めます（正方行列の
積を数個、両デバイスで計時します。一度きりで数ミリ秒）。別のマシンで
測った表ではなく、実際に載っているCPUとGPUの力関係で分かれます。
`use_cpu()` / `use_gpu()`を呼べば — 最初のテンソルを作る前でも —
以降の演算はそのデバイスに固定されます。
...
`fn`自身の戻り値は捨てられます。セッション中、エンジンは実行する演算ごとに
その名前のスコープ（`dot`・`sum_to`・`layer_norm`・`adam_step`など）を開き、
`.backward()`は`backward`と、VJPごとにCulebra側の演算名（`Dot`・`LayerNorm`）
のスコープを開きます。入れ子のスコープは`/`でつないだパスになります。
バックエンドが行うカーネル起動・ホストとデバイス間のコピー・ブロックする
待ちは、そのとき一番内側で開いているスコープに帰属します。
...
| フィールド | 意味 |
| --- | --- |
| `kind` | スコープ自身の行は`'scope'`。その下の仕事は`'launch'`・`'transfer'`・`'wait'` |
| `path` | スコープのパス（`'step/backward/Dot/dot'`） |
| `kernel` | 起動はカーネル名。転送は`'h2d'` / `'d2h'`。それ以外は`''` |
| `count` | スコープに入った回数、または起動 / 転送 / 待ちの回数 |
| `host_ms` | スコープ: 中身を含む経過時間。転送と待ち: ホストがブロックされた時間 |
| `device_ms` | 起動: デバイス上のカーネル時間の合計（バックエンドが計れる場合） |
| `device_timed` | その行の起動のうち`device_ms`に含まれる数 |
| `bytes` | 転送: 転送したバイト数 |

行はパスごとにまとまり、パスはスコープの経過時間の降順、各パスの中は
スコープ自身の行が先で、起動はデバイス時間の降順です。`device_ms`は
CUDAでは起動ごとに計ります。Metalは起動ではなくコマンドバッファ単位でしか
計れないので、起動の行は`device_timed`が0で、バッチの時間は行に含まれ
...
`at`は`line`/`col`より前、「そのノードが何であるか」を言う引数の直後にある。
だからノードを手に持っているフロントエンドは位置引数で書ける:
...
3つとも省略できる。何も渡さずに作ったノードは1:1になる。ソース行が生んだ
...
| `m.unary(op:, operand:, at:)` | `op`は`'neg'`/`'bitnot'`/`'not'` ——`not`は条件が見るのと同じ真偽判定を反転したBoolを返すので、Bool以外の値にも答える —— または`wrapi8 wrapi16 wrapi32 wrapu8 wrapu16 wrapu32`のいずれか —— Longをその幅に切り詰め、符号拡張(`wrapi*`)またはゼロ拡張(`wrapu*`)して戻す。固定幅の`int`/`uint`(C#/Java/Goのサブセットなど)を下ろすフロントエンド向け。オペランドは既にLongでなければならず、これらはどれも`lt`/`le`/`gt`/`ge`/`div`/`mod`/`shr`が自前のwrapなしに前提とする正規化済みの形にスロットの値を保つ |
...
範囲の開始位置は戻ってはならない。同じバイトから始まる2つの語は、1つの語位置に重ねた
候補になる——複合語とその構成語を並べて出せば、どちらで検索しても文書が見つかり、句検索
もどちらを通っても続く——。どの候補でヒットしても、ハイライトの範囲はその位置に最初に
出した語のもの。あとから始まる語は、範囲が重なっていても次の位置になる。問い合わせ側も
同じsplitterが走るので、こう割れた語は各位置に候補を持つ句として検索される。
...
何も見つけられない。ファイルはその形を記録していて、`load`は食い違いを断る
（[保存と読み込み](#保存と読み込み)を参照）。
...
| 名前 | モデル | 大きさ | ライセンス |
|---|---|---|---|
| `ja-ud-gsd` | cpp-segmentlibの参照モデルv0.2.0。UD Japanese-GSDで訓練 | 2.1 MB | CC BY-SA 4.0 |
...
写しのファイル名にはバージョンが入る（`ja-ud-gsd-v0.2.0.mod`）ので、上流がモデルを
...
ファイルは、それを作った解析器がどういう形だったかを記録している。`load`は形の違う
解析器を断る:
...
記録される形は粗い。ファイルが正直に書けるのはそこまでだからで、内訳は、どの種類の
splitterが語を切ったか（組み込みの規則か、モデルか——ファイル名と大きさで名指す——、
wrapしたC++のクラスか、culebraのclosureか）、normalizerが組み込みのままか、filterが
何本走ったか。culebraの関数には処理をまたいで残る同一性がないので、closureはどれも
同じに見える。停止語の一覧を同じ長さの別のものに差し替えた場合は捕まらない。捕まるのは
名前のある食い違い——モデルで作った索引をモデルなしで開いた、filterが1本消えている——で、
この種の間違いはたいていその形をしている。

ファイルには書いたときの形式が記録されていて、`load`は形式の違うファイルを誤読せず
`SearchError`で弾く。v0.7.0より前に保存した索引は、`Search.Index.new`で作り直す必要がある。
