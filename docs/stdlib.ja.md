...
**ウィンドウが閉じる前の確認。** フレーム自身の閉じるボタンを押しても、
ウィンドウはそのままでは閉じません。ページが`window.__culebra_before_close__`
を定義していれば、そのフック単独が閉じるかどうか・いつ閉じるかを判断します
（confirmダイアログや未保存チェックなど）。準備ができたら`window.__culebra_close__()`
を呼んでください — ページ内のquitボタンから直接呼んでもよい関数で、
`Desktop.run`かどうかを問いません。`__culebra_before_close__`を定義して
いないページは、これまで通り即座に閉じます。
