...
やメソッドが常に優先されます。また、第一引数の型注釈が`x`を
受け取れない`f`は候補になりません:
...
すべてに当てはまる。`Scene.Image(4, 4)`は`Scene.Image.new(4, 4)`と、
`Channel()`は`Channel.new()`と同じ呼び出しになる。§4の索引が`.new(...)`と
...
**Isolate** — Isolate.spawn(fn, *args) -> handle; h.join(); h.poll(); Channel.new(cap = 1) -> (tx, rx); tx.send(v); tx.clone(); rx.clone(); tx.drop(); rx.drop(); rx.recv(); rx.try_recv(); rx.drain(max = nil); Channel.fan_in(items: [rx]) -> rx; Channel.fan_in(items, fn) -> rx; Parallel.map(items, fn, limit = <コア数>); Parallel.each(items, fn, limit = <コア数>); Parallel.map_settled(items, fn, limit = <コア数>); Parallel.race(items, fn, limit = <コア数>); Signal.notify(tx); Signal.reset(); SharedBuffer.new(count, type: Class) -> buffer; SharedBuffer.file(path, count, type: Class) -> buffer; SharedBuffer.shared(count, type: Class) -> buffer; buffer.with_lock(fn)
