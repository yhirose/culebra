...
**Peg** — Peg.compile(grammar) -> Peg; Peg.compile(grammar, start); Peg.compile(grammar, start, optimize); Peg.compile(grammar, start, optimize, packrat); Peg.check(grammar) -> Nil; Peg.check(grammar, start); p.parse(text); p.test(text) -> Bool; Peg.parse(grammar, text); Peg.parse(grammar, text, start, optimize, packrat); Peg.test(grammar, text); Peg.test(grammar, text, start, packrat); Peg.walk(node) -> Iterator; Peg.find(node, name); Peg.find_all(node, name) -> [Node]; Peg.str(node) -> String
