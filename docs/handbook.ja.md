...
引数をフィールドに写すだけのコンストラクタは、それをパラメータリストに
書ける。`.x`はフィールド`x`にも格納するパラメータ、`.x?`はフィールド
宣言のデフォルトを取る省略可能なパラメータ。本体には構築に必要な
残りだけを書けばよく、空でもよい。
...
パラメータがフィールドを宣言することもできる (クラス本体に`x`、`y`を
宣言せず`new(.x: Float, .y: Float) {}`と書く)。小さなレコード的なクラス
ならこれだけで済む。規則の全体は[language.md §10](language.md)
「`class` sugar」を参照。
...
| 端末・グラフィクス・音 | `Term`、`Canvas`、`Scene`、`Audio`、`Desktop` |
...
`Runtime`とそのスコープは省いてよい。書かなければエンジンを最初に
使った時点でスレッドローカルの既定が作られ、エンジン1つのホストには
それで足りる。1スレッドに独立したエンジンを複数置くときだけ書く
（[`deployment.ja.md`](deployment.ja.md#同一スレッドで複数スクリプトを動かす)）。
