...
ブラウザのPlaygroundだけはバイナリではありません。特定のプログラムを開くURL、
つまりShareボタンが書き出す形式そのものは
[§7](#7-playgroundへのリンク) が規定します。
...
7. [Playgroundへのリンク](#7-playgroundへのリンク)
   * [プロジェクト](#プロジェクト)
   * [テキスト](#テキスト)
   * [ページ](#ページ)
   * [Shareボタン](#shareボタン)
...
これらは1回の実行のあいだだけ、どのレーンにもある標準ライブラリの名前に
上乗せする形で束縛されます。
...
culebra test --doc --jit --jobs 8 docs  # ... を 8 プロセスで
```
...
**ファイルは1つで1つのプログラムです。** ファイルごとに独立したスコープが
与えられ、ランナーは次のファイルを読み込む前にそのファイルのテストを走らせ
ます。トップレベルに書いたものはそのファイルのもので（`mut range = 5`と
書いても他のファイルでは`range`は関数のまま）、クラス・オーバーロード・
`Sys.script`・`Embed.dir(...)`が基準にするディレクトリも同様です。ある
ファイルで宣言したフィクスチャは別のファイルからは見えません。パラメータ
経由で渡すか、必要なファイルごとに宣言してください。`import`も使えて、
スクリプトのときと同じくモジュール一式が1つの単位として走ります。

ファイルのトップレベルはスクリプトではなく**セッション**のスコープで走り
ます。スコープの読み方はどちらでも同じで、関数の中の裸の代入は、既に何かが
その名前を宣言していない限りその関数のローカル宣言です。ただし帰結が1つ
あります。ランナーは`test` / `parametrize`とマッチャ群をファイルより先に
宣言するので、それらの名前は宣言済みとして数えられます。関数の中からその
1つに裸で代入すると再代入になって拒否されます。スクリプトなら同じ行が
ローカルを宣言するところです。
...
`files`は走ったテストファイルの本数です（`--list`がパスを並べる本数と同じ）。
`--doc`実行が数えるのはファイルではなくブロックなので、その`run_end`に
`files`はありません。
...
` ```culebra `ブロックを取り出し、新しいVMランタイムで実行して、下記の
...
- `# doctest: <directive>`（ブロック先頭行）— モード:
  ...
  - `compile-only` — 構文検査のみ
  - `jit-only` / `aot-only` — バックエンド指定

ブロック間は独立で`setup` / `teardown`はありません。複数ステップが必要な
例は1ブロックに収めてください。ランナーが現在解釈するのは`skip`のみで、
`compile-only`とバックエンド指定は予約済み（該当ブロックは通常どおり実行
されます）。

`--jit`は同じブロックをVMではなくLLVM JITで走らせます。ドキュメントの例が、
それを実行しうるすべてのエンジンで同じ出力・同じthrowになることを確認
できます。`--doc`を付けない`culebra test`（ユニットテストのランナー）は
VMで走ります。

`--jobs <n>`はブロックを`n`個のプロセスに分けて実行します。各プロセスは
ブロック一覧の連続した区間を担当し、親がその出力をソース順に並べ直すので、
レポートの見え方は逐次実行と同じです。JITレーンは1ブロックにつき1つの
LLVMモジュールをコンパイルするため、このフラグはそこを狙ったものです。
`--bail`と`--list`は逐次のままです。互いに競走するプロセス間では
「最初の失敗で止める」が意味を持たず、列挙は何も実行しないためです。

例を書くときに知っておく価値のある帰結が2つあります。式を単に置いても
何も出力されないので、検証される例は`inspect(...)`か`println(...)`を
通す必要があります。そしてCulebraでは`#`と同様に`//`も行コメントを
...
CIがゲートにできるよう非ゼロで終了します（0 = 問題なし、1 = 警告のみ、
...
`.cul`を1つも含まないパスはcleanではなくexit 2になります。ディレクトリ
の打ち間違いがゲートを素通りしないためです。

`test_*.cul`（`culebra test`が探索するのと同じ規約）という名前のファイルは、
そのランナーが走らせるのと同じ扱いでlintされるので、`test`と`parametrize`は
束縛済みとして数えられます。それ以外のファイルでは従来どおり未定義の名前で
あり、`culebra lint .`は1回のパスで両方を正しく判定します。
...
- **エラー** — 確実に失敗する健全な集合: 場所が不正な`break` / `continue` /
  ...
  部分集合）。これらは元々どの実行も中断させるもので、`lint`は最初の1件で
  止まる代わりに全件を一度に見せます。
- **警告** — 実行を止めない助言:
  - **未使用のローカル変数** — 関数内の`let` / `mut`で、一度も読まれない
    もの。
  - **未使用のトップレベル束縛** — モジュールが読みも再exportもしない
    トップレベルの`let` / `mut`。関数・クラス・enum・trait宣言はモジュールの
    export面なので対象外です。
  - **未使用のimport** — `import`した名前をモジュールが使っていない。
  ...
  印で、決してフラグされません。**パラメータは対象外です**: Culebraで
...
`culebra lint --fix <paths...>`は未使用import行を機械的に削除します。
無人で自動修正して安全なのはこの警告だけです。死んだ`import`を消しても
振る舞いは変わり得ない一方、未使用の`let`/`mut`は初期化式が副作用を持ち
うるからです。他の警告はすべて報告のみです。編集後は修正済みソースを
再パース・再lintし、importが消えて新しいエラーも出ていないと確認できた
場合にだけ書き出します — `culebra fmt`と同じ再パースの安全網です。

削除は行単位なので、対象になるのはimport 1つだけがある行に限られます。
`;`で他の文とつながったimportは報告のみで手つかずのまま残します:
その行を消すと隣の文まで巻き添えになり、しかも文が1つ消えてもファイルは
同じようにパースもlintも通るため、再チェックでは気づけないからです。
...
`culebra fmt [paths...]`はソースを1つの正準スタイルに整形します:
演算子まわりの空白の正規化、2スペースインデント、ブレースブロックの複数行
...
プロジェクト全体を整形し、`culebra fmt --check .`でCIをゲートできます。
`.cul`を1つも含まないパスはエラー（exit 2）なので、パスの打ち間違いは
...
1つでもあれば終了コードは1なので、書き換えた後でもCIのゲートになります。
整形結果が標準出力に出るのは、他の出力モードを何も指定しなかったときだけです。

`-`はstdinを読み、単独でのみ使えます: ファイルパスとの併用（入力2つに
stdout 1つ）や`-i`との併用（書き換える対象が無い）は、どちらかを黙って
優先するのではなくexit 2になります。パスを指定しない`culebra fmt -i`も
同じ扱いです。未知のフラグもファイル名と解釈せずエラーにします —
`culebra fmt --wirte app.cul`はexit 2で何も整形しないので、打ち間違いが
成功した実行に見えることはありません。

コメントは保存されます: 前置コメントは導入する文の上に、行末コメントは同じ
行に残り、文の間の空行1行も保たれます（連続する空行は1行に畳まれます）。
match / condのアーム、クラス・trait・enumのメンバ、分配パターン、
パラメータリストはすべて正規化され、長い二項式やメソッドチェーンは行幅で
折り返されます。`if` / `while` / `match` のヘッドは括弧を持たないため、
折り返しは自身のチェーンの `||` / `&&` / `??` の継ぎ目だけで起き（比較の
左右の間では折りません）、継続行は本体より1段深くインデントされます。
コメントを持つオブジェクト/セットリテラルは1要素ずつ行を
...
Culebraは [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol/)
(DAP) サーバを同梱しています。これにより、DAP対応エディタ — VSCode・Vim
(vimspector)・Zed・Emacs (dap-mode)・Helixなど — でブレークポイント・
ステップ実行・変数の確認をビジュアルに行えます。1つのアダプタで全エディタに対応します。
...
- **`culebra`を実行できること** — `PATH`を通すか、下記の各設定で絶対パスを指定。
  （動作確認: `echo | culebra dap`がDAP入力待ちでハングすればOK、`Ctrl+C`で抜ける。
  エラーになる場合はまずパスを直す。）
- **デバッグはVMで動く** — `--jit`を付けない。アダプタはプログラムを
  バイトコードVMで実行する。JIT/AOTは機械語にコンパイルされソースレベルデバッグ
  できない。

**仕組み。** アダプタはVMのバイトコードデバッグテーブルを読んでブレークポイント・
`debugger`文・ステップ時に一時停止します。その間DAPループがエディタの要求に
...
- **watch / evaluate**: 選択フレームで式を評価（watchパネル・hover・デバッグ
...
どの統合も`.cul`を同一の規約でハイライトします（生成された1つの語彙リスト
に基づく）。言語が最初から提供する大文字始まりの名前、つまり`Canvas`のような
標準ライブラリのnamespaceや`Long`のような型注釈名は組み込み型として着色し、
それ以外の大文字始まりの名前はプログラム自身が宣言したものとして着色します。
自分で書いたclass・trait・enumは、言語が知っている名前ではなく自分のものとして
読めます。Playgroundのエディタも同じ区別をします。
...
VSCodeは`.cul`のハイライトと`culebra`デバッグタイプ登録のための小さな拡張が必要です
...
1. 拡張をインストール — `culebra init`、またはソースチェックアウトから:
   ...
   でインストールします＝VS Codeが公式にサポートする方法。（`~/.vscode/extensions`に
   フォルダを直接コピーする方法は**非サポート**で、認識されないことが多い。）`culebra`が
   `PATH`上にあればデバッグアダプタ設定に絶対パスを埋め込みます。`code-insiders`/`cursor`/
   `codium`でも動作し、いずれのCLIも無ければExtensionsビューから`.vsix`を入れる手順を
   案内します。シンタックスハイライトは`.cul`を開くだけで有効（以降の手順はデバッグ用のみ）。
   文法のキーワード一覧は`just sync-grammar`がパーサから生成（Vim構文ファイルと同一ソース）
   するため言語からドリフトしません。
2. VSCodeを**完全終了**（<kbd>Cmd</kbd>+<kbd>Q</kbd>）して再起動 — 入れたての拡張は
...
> VSCodeで開き`extensionHost`のlaunch構成で <kbd>F5</kbd> を押すと、拡張がライブロード
...
プロジェクトルートに`.vimspector.json`を置きます（vimspectorはプロジェクト
単位の設定なので、culebra側からは配布できません）。Vimのシンタックスファイルを
...
vimspectorは**デフォルトでキーマッピングを一切設定しない**ため、`vimrc`に
...
`<F10>`/`<F11>`/`<F12>`でstep over/in/out、`<F3>`または`:VimspectorReset`で終了）。
gadgetのインストール（`:VimspectorInstall`）は不要 — 上記`command`をstdioで
直接起動します。Vimは`+python3`ビルドが必要です。
...
culebra docs stdlib Scene        # その中の1章をまるごと表示
culebra docs -g 'Math.wrap'      # 一致したセクションを表示
culebra docs stdlib -g 'wrap'    # 1トピックだけ検索
culebra docs --ja ...            # 日本語版
```

終了ステータスはgrepの慣習に従います。`0` = 何か表示した、`1` = 一致
なし、`2` = 使い方の誤り。おかげで実在確認が、出力を読まずに1行で
書けます:
...
ような署名の断片 — は拒否せずリテラルとして検索し、その旨をstderrに
出します。
...
一致セクションが8個を超えると出力は見出しの索引に降格するので、広い
パターンのコストが1画面で収まり、リファレンス全体にはなりません。
見出し一致の上位数件はそれでも全文で表示されます。`--full`はこれを
上書きし、`--at <行>`は1セクションを打ち切りなしで表示します（行
番号は各ヒットの上に出るロケータのものです）。

セクションは構造上小さく保たれている（中央値21行）ので、ヒットは
通常まるごと表示されます。60行を超える分は`--at`への案内をつけて
打ち切ります。

### 章を名指しする

2つめの引数はトピックの中の章を名指しし、その章をまるごと — 前書きも
小節も、打ち切りなしで — 表示します。これは`-g`にはできないことです。
検索は見出しごとに切るので、その仕組みの動かし方を書いた本文と、実際に
動かす署名とが、別々のヒットとして返ってくるか、両方が索引になります。

```
culebra docs stdlib Scene        # `Scene`の章ぜんぶ
culebra docs stdlib Webview      # `Desktop` / `Webview`はどちらの名前でも
```

一致は大小を区別せず、章番号も無視するので`scene`でも`Scene`でも
引けます。どの章も名乗っていない名前はexit 1になりますが、その際に
実在する章の名前を並べるので、うろ覚えの名前はそこで直せます。

`-g`を添えると、章を表示する代わりに検索の範囲がその章に絞られます。
パターンが広すぎるとき「トピックを1つに絞る」の次に取る手です:

```
culebra docs stdlib Scene -g 'mesh'    # Sceneの節のうち、それに触れているものだけ
```

### どのトピックを読むか

`culebra docs quick-guide`が凝縮パックです。構文・他言語から転移しない癖・
言われなくても手が伸びる範囲の署名が、プロンプトに収まる1ファイルに
入っています。これは検索するものではなく、culebraを書く前に読むもの
です。状態を持つ仕組み — `Scene`、`Canvas`、`Net`、`PEG`、`CodeGen`
など — は署名だけでは動かし方が分からないので、一覧にせず名前だけを
挙げてあります。必要になった時にその章を表示してください。残りは
...
PlaygroundはCulebraをブラウザで実行します。何を開くかは全てURLで決まります。
リンクは互いに独立した3つの部分から成ります。テキストが何と並んで動くかを指す
**プロジェクト**、**テキスト**そのもの、そして受け取った人が着く**ページ**です。
どれも省略できます。
...
| パラメータ | 開くもの |
|---|---|
| `?example=<title>` | Examplesメニューでの正確なタイトルで指すカタログの例 |
| `?src=<https URL>` | Webから取得する`.cul`ファイル |
| `?args=<command line>` | プログラムに渡す引数。プロジェクト自身の引数を上書きする |

`?example=`はPlayground自身の例をメニューの表記どおり（URLエンコードして）
名指しします。同伴ファイルも一緒に来ます。自分専用のディレクトリに置かれた例は
そのディレクトリ全体を連れてくるので、importもデータファイルもcheckoutと同じに
解決されます。

`?src=`はGitHub上のファイルを、人がコピーする3つの綴りのいずれでも受け取ります。
...
そのファイルが置かれたディレクトリが再帰的に付いてきます。取得はjsDelivr経由です。
GitHub自身のホストはブラウザにディレクトリ一覧を返さないためです。500ファイルまたは
20 MBを超えるディレクトリはその旨のメッセージとともに拒否されるので、大きな
リポジトリのルートに置かれたプログラムはまず自分のディレクトリへ移す必要があります。
それ以外のhttps URLは、周りに何も伴わない単一ファイルとして取得されます。

エントリのパスがそのプログラムの`Sys.script`であり、`import`もそれを基準に
解決されます。リンクされたプログラムはcheckoutで使うのと同じ式で自分のファイルを
見つけます。プロジェクトが無ければ、テキストは`main.cul`として単独で動きます。

### テキスト

`#code=<payload>`がソースそのものを運びます。ペイロードはUTF-8のソースを
base64urlにしたもの（`+`と`/`の代わりに`-`と`_`、パディング無し）で、そのほうが
短くなるときはgzip圧縮されます。どちらの形式かを告げるパラメータはありません。
gzipのマジックバイト2つはCulebraのソースの先頭には現れ得ないので、ペイロードは
自己記述的で、デコーダはそれを読みます。上限は入口と出口の両方にあります。約100万文字を
超えるペイロードは読まずに拒否され、展開して4MBを超えるソースは展開の途中で打ち切られます。
gzipは約1000倍まで膨らむので、入口の上限だけでは数百MBを要求するリンクが通ってしまいます。

クエリ文字列ではなくフラグメントに載るのは、フラグメントがサーバまで届かないからです。
GitHub PagesのCDNは8 KBを少し超えたあたりでリクエスト行を拒否し、プログラム全体を
無圧縮で符号化するとそれを超えることがあります。`?code=`は古い綴りで、既に出回って
いるリンクのためにフラグメントの次に読まれます。

プロジェクトとテキストの両方がある場合、テキストがプロジェクトのエントリソースを
置き換え、周辺のファイルは引き続きプロジェクトが供給します。編集された例はこの形で
共有されます。デコードできないペイロードはブラウザのコンソールに報告され、リンクは
プロジェクトのソースにフォールバックします。どちらも無ければ、Playgroundは
プロジェクトのエントリソースを、無ければ前回訪問時に保存された下書きを、それも
無ければHelloの例を開きます。

### ページ

受け取った人が見るものは2つのパラメータで決まります。`?layout=`がページのどこまでを
組むか、`?view=`がどのペインを軸にするかです。

| パラメータ | 見えるもの |
|---|---|
| `?layout=` 無し | Playground全体 |
| `?layout=editor` | エディタ・Run/Stop・ペイン。サイトのヘッダ、examplesのメニュー、引数欄、New/Shareボタンは出さない |
| `?layout=view` | ペイン1つだけ |
| `?view=output` | Outputペイン（既定） |
| `?view=tui` | TUIペイン |
| `?view=canvas` | Canvasペイン |
| `?split=top` | ビューをソースの横ではなく下に置く |

組み合わせられます。後者が前者の残したものを名指しします。`?layout=view`と
`?view=canvas`で周りに何も無いゲーム、`?view=tui`ならターミナルプログラムの
同じ形、`?layout=editor&split=top`はソースとその下の出力を、自前のブランディングを
持つページの中に置いた形です。`?embed=1`と`?embed=editor`は`?layout=editor`の、
`?embed=output`は`?layout=view`の古い綴りで、どれも読み続けます。
...
タブ列が無い`?layout=view`のページです。後者では`?view=`が唯一の指定手段になります。

`?run=1`はRunを待たず、エンジンの準備ができ次第プログラムを開始します。発火は
一度きりです。プロジェクトやテキストの読み込みに失敗したリンクは、代わりに
復帰したものを自動実行しません。

`?layout=view`のページにはツールバーが無いので、プログラムが動いていない間は
ペインの上に再生ボタンが出ます。`?run=1`無しのリンクで訪問者が開始する手段であり、
終わったあと誰でも再生し直す手段でもあります。停止はありません。自分では終わらない
プログラムはタブと一緒に終わります。

このページは空のペインではなくプログラムそのものから始まります。`?view=output`なら
ソースを読み取り専用・ハイライト付きで表示し、最初の実行で出力に置き換わります。
`?view=canvas`と`?view=tui`ならプログラムの最初の画面で、これはそこまで実際に走らせて
止めることで得られます。Canvasのプログラムは最初の`Canvas.present()`で保留され、
TUIのプログラムは最初の画面の末尾にある`Term.read_key`で自分から止まります。再生ボタンは
2度目の実行を始めるのではなくその保留を解きます。クリックである以上それは音のロックも
解きます（ブラウザはページが一度クリックされるまで音声をロックするので、`?run=1`の
ページは訪問者が触るまで無音です）。最初の画面に到達するまでプログラムは実際に走るので、
その途中で何かをするリンクは、訪問者が再生を押した時ではなくページを開いた時にそれをします。

### Shareボタン

Shareはこれらの選択肢（`Layout`・`View`・`Split`・`On open`）を持つメニューを開き、
選んだとおりのリンクをコピーします。それ以外はページから決まります。プロジェクト、
プロジェクト自身のものと異なるときの引数、そして編集されたときのテキストです。
編集されていない例は数KBのフラグメントではなく短い名前で共有されます。既定値は
素のPlaygroundそのものなので、何も変えなければリンクは4つのどれも載せません。

Splitは効く場所でだけ出ます。ビューだけで組んだレイアウトには横に並べるソースが
無いので、その行は静かになり、リンクもそれを載せません。

Examplesのメニューをプレースホルダに戻すことは「このテキストはもうその例ではない」
という意思表示で、名前と一緒にプロジェクトも外れます。以後そのテキストは、例の同伴
ファイルも引数も伴わず`main.cul`として単独で動き、リンクも何も名指ししません。
