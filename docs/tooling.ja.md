...
受け取った人が見るものは2つのパラメータで決まります。`?embed=`がページのどこまでを
見せるか、`?view=`がどの出力ペインを見せるかです。

| パラメータ | 見えるもの |
|---|---|
| `?embed=` 無し | Playground全体 |
| `?embed=editor` | エディタ・Run/Stop・ペイン。サイトのヘッダ、examplesのメニュー、引数欄、New/Shareボタンは出さない |
| `?embed=output` | 出力ペイン1つだけ |
...
タブ列が無い`?embed=output`のページです。後者では`?view=`が唯一の指定手段になります。

`?run=1`はRunを待たず、エンジンの準備ができ次第プログラムを開始します。発火は
一度きりです。プロジェクトやテキストの読み込みに失敗したリンクは、代わりに
復帰したものを自動実行しません。

`?embed=output`のページにはツールバーが無いので、自前の操作を2つ持ちます。
プログラムが動いていない間はペインの上に再生ボタンが出ます。`?run=1`無しのリンクで
訪問者が開始する手段であり、終わったあと誰でも再生し直す手段でもあります。動いている
間は隅に停止ボタンが出ます。自分では終わらないプログラムを、タブごと閉じずに
止められるようにするためです。

### Shareボタン

Shareはこの3つの選択肢（`Embed`・`View`・`On open`）を持つメニューを開き、選んだ
とおりのリンクをコピーします。それ以外はページから決まります。プロジェクト、
