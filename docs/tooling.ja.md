...
WebAssembly JSPIの無いブラウザ（27より前のSafari）は、CanvasやTUIのプログラムを最初の画面で
保留することも入力を渡すこともできず、`?layout=view`のページには停止もありません。そこでは
`?view=canvas`と`?view=tui`のページは`?run=1`の有無にかかわらず何も実行せず、`?view=output`と
同じようにソースを表示して、その上にブラウザに足りないものを示す一文を添えます。再生ボタンは出ません。
...
- **ホバー** — 文書の中で宣言した名前には、その宣言と推論した型を表示します
  （`parts: Array<String>`）。それ以外は、リファレンスに見出しがある名前について、
  その項目を表示します。標準ライブラリの関数や定数（`Math.abs`、`Math.pi`）と、
  グローバル関数（`type_of`）が対象です。値に対して呼ぶメソッド（`xs.size()`）
  には何も表示しません。どのメソッドになるかは値の型で決まるためです。
- **補完** — `value.`の後では、値の型から分かる範囲でメンバーを出します。
  組み込み型のメソッド、クラスのインスタンスのメソッドとフィールド（`self`を
  含む）、オブジェクトリテラルのキー、namespaceのメンバー、標準ライブラリの
  関数が返すクラス（`Regex.compile(p).`）のメソッドが対象です。その後に、
  UFCSで呼べる関数を続けます。名前だけを打っているときは、その位置で見える
  名前を出し、続けてグローバルを出します。型はリテラル、戻り値の型、型注釈、
  代入から求めます。関数を受け取る組み込みメソッドでは、その関数の引数を受け手の
  要素型とし（`words.map(|w| w.`）、結果の型は関数が返すものから決めます。
  型注釈のない引数は、文書の中の呼び出しが渡す値から型を求めます。ただし、その
  関数がexportされている場合や、値として渡されている場合は求めません。importした
  名前は、importしたモジュールの宣言から型を求めます。型が分からない値には、
  UFCSで呼べる関数だけを出します。
- **定義へ移動・参照の検索・ハイライト** — 変数、引数、関数、クラス、enum、
  import、エフェクト操作が対象です。名前の見え方は
  [`language.ja.md` §6](language.ja.md#6-変数とスコープ)の規則に従います。
  ブロック、ループ本体、matchの腕はそれぞれ自分の名前を持ち、クロージャは
  外側の関数の変数を、宣言の位置にかかわらず参照できます。直接の呼び出しに
  書いたキーワード引数（`f(count: 2)`）は、その引数への参照として数えます。
  importした名前`Alias`について`Alias.member`の定義へ移動すると、そのモジュールの
  トップレベルにある`member`の宣言を開きます。
- **アウトライン** — トップレベルの宣言を、クラス・enum・traitのメンバーと
  一緒に並べます。
- **名前の変更** — 文書の中の出現をすべて書き換えます。オブジェクトの省略記法は
  キーを残し、`{sum}`は`{sum: total}`になります。書き換えきれない場合は、
  サーバが変更を断ります。
  - その名前がexportされている。importしているモジュールが追従しません。
  - 同じ名前のメソッド呼び出し（`value.name(...)`）がある。UFCSでその関数に
    届く可能性があります。
  - 新しい名前がキーワードである。または、変更する名前を使っている場所で、
    すでに別のものを指している。
  - 文書が今の状態ではパースできない。

位置はプロトコルの既定に従います。行は0から数え、列はUTF-16のコード単位で
数えます。パースできない間は、最後にパースできた版をもとに、定義・参照・
ハイライト・アウトラインを返します。
