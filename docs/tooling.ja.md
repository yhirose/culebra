...
実行ファイルが、テストランナー・リンタ・フォーマッタ・デバッグアダプタも
兼ねています。この文書はその 4 つの開発用サブコマンドのリファレンスです。
...
出力モードは組み合わせられます: `culebra fmt -i -l .` は全ファイルを書き換え
**かつ**変更したファイルを列挙し（`gofmt -l -w` と同じ）、`-i --check` は同じ
ことを出力なしで行います。`-l` / `--check` を付けた場合、整形が異なるファイルが
