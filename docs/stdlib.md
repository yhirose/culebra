...
| `m.unary(op:, operand:, at:)` | `op` is `'neg'`/`'bitnot'`/`'not'` — `not` yields the Bool inverse of the same truthiness a condition tests, so it answers for every value rather than only for a Bool — or one of `wrapi8 wrapi16 wrapi32 wrapu8 wrapu16 wrapu32` — truncates a Long to that width and sign- (`wrapi*`) or zero-extends (`wrapu*`) it back, for a front end lowering a fixed-width `int`/`uint` (a C#/Java/Go subset, say): the operand must already be a Long, and every one of these keeps a slot's value in the normalized form `lt`/`le`/`gt`/`ge`/`div`/`mod`/`shr` already assume without a wrap of their own |
...
| `m.switch_subject(node:)` | a `switch` node's subject, as a node id |
| `m.switch_arm_count(node:)` | how many `(key, body)` arms `node` has |
| `m.switch_key(node:, index:)` / `m.switch_body(node:, index:)` | the `index`-th arm's key or body, as a node id |
| `m.switch_has_default(node:)` | whether `node` has a default arm |
| `m.switch_default_body(node:)` | its default arm, as a node id; fails if there is none |
| `m.field_slot(node:)` / `m.field_name(node:)` / `m.field_receiver(node:)` | for a `fieldget` or `fieldset` node: its slot, its diagnostics-only name, and its receiver as a node id |
| `m.field_set_value(node:)` | a `fieldset` node's value, as a node id |
...
| `m.func_singleton(func:)` | whether `set_singleton` was called on it |
...
| `p.run(rt: nil, max_call_depth: 10000, natives: nil)` | runs the compiled program. `rt` is a `CodeGen.Runtime` to run on, or nil for a throwaway heap; `max_call_depth` bounds the frame stack. `natives` supplies what the module's `declare_native` names refer to — an Object mapping each declared name to an ordinary culebra `Function`, which the program calls like any other value. Every name the module declared must be there (`IrError` naming the one that is not, before the run starts); a name in the table the module never declared is simply unused, so one table can serve several programs. Values crossing that boundary are copied, and only `nil`, `Bool`, `Long`, `Float` and `String` can: the program's heap and culebra's own are collected independently, so an array, object, map, closure or coroutine would need the two collectors to agree about it. Anything else — as an argument, or as what the function returns — is a `TypeError` naming the slot, catchable by the program's own `make_try`. So is a wrong argument count (checked against the function's own arity), and so is whatever the function throws: a `nil`/`Bool`/`Long`/`Float`/`String` arrives as itself, anything else as the `{kind, message, line, col}` object a culebra error carries |
...
| `rs.closure(m:, builder:, target:, at:)` | a closure of `target` built in `builder`'s frame, as one node |
| `rs.capture_map(m:, builder:, target:)` | that closure's forwarding table alone, for a front end assembling the node itself |
...
runtime dispatcher (`stdlib_rt.h::kNsRows_*`) while the syntactic
