...
走らない検査は落ちない検査である。このツリーが被覆を失った経路のうち2つは、
誰も見ていない免除だった。

**`# doctest: skip`。** skipされたブロックは一度も実行されないので、文書に
書かれた形が動かなくなっても文書には残り続ける。実際に2度起きた
（`test("name", fn)`の形と、フォーマットの例。どちらも人が読んで気づいた）。
skipの正当化は2通りだけになった: そのブロックがそもそも走れないか、
なぜここで走らせないかを書いてあるか（`# doctest: skip — ウィンドウを開く`）。
`check_doctest_skips.sh`が理由のないskipを executor で走らせ、きれいに走った
ブロックはゲートを落とす——skipを外すか、読者が知りたい理由を書く。
見つかった338本のうち92本はウィンドウ・サーバ・監視・stdin・ネットワーク・
外部プログラム・マシン固有のパスで、14本は単に走る例示だった。後者は今は走って
いて、その結果§12の`nobreak`の節は初めて実行される例を持った。

92本のうち6本はここではなくCIが見つけた。これがこのレーンを全プラットフォームで
走らせる理由でもある——あるマシンで走れない例が、別のマシンでは走る。
`Net.listen(7000)`とそのacceptループはランナーではハングし、このラップトップでは
失敗した。`Proc.spawn(["python", ...])`は`python`がPATHにある環境では走り、
`python3`しかない環境では失敗する。**手元のマシンで成立するskipは、成立するskipでは
ない。**

**`examples/`。** `examples/`配下の30スイート・328 assertionは、どのゲートにも
繋がっていなかった。`vm2gol-v2`のスイートはunit runnerがVMに移って以降
走っていなかった——`culebra test`が`import`を含むファイルを全部拒否していた
ためで、しかもそれを誰も言わなかった。`run_examples_sweep`がこれを executor
1プロセス・3秒で回す。全レーンで走るようになったので、`tests/*.cul`と同じ
optional-namespaceの規則も適用する。
