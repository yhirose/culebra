...
| ゲート | 3レーンの状況 |
|---|---|
| 生成difftest corpus | 15,845ケースをinterp / `--jit` / `--vm`で。差異0、**skip 0**（`tools/difftest/vm_skip_ceiling.txt`はratchetで現在0） |
| curated `tools/bench/vm_cases/` | 172ケース。`--vm`と`--vm-llvm`をそれぞれinterpと差分比較し、同じスイープを`CULEBRA_GC_STRESS=1`下でもう一度 |
| `tests/*.cul`の対称性とisolateスイート | 4レーン: interp、`--jit`、`--vm`、`--vm-llvm` |
| `just doctest` | ドキュメント461ブロックを3エンジン全部で（`just doctest LANE=interp\|vm\|jit\|all`） |
...
opcodeは138個、`include/vm.h`は約14.7k行。面積の大半を担った構造
...
### 11.4 sliceの外に残っているもの

Phase 2のバーはゲートであり、対称性ゲートはslice外rejectを失敗
ではなく可視のSKIPとして数える。204本の`tests/*.cul`のうち11本が
今もその出口を通る。4クラスタある:

| 形 | テスト数 | 備考 |
|---|---|---|
| 型注釈付きの変数宣言（`let x: Long = 5`） | 4 | 宣言側の注釈がreject。同じ型名はパターンとパラメータでは既に動く |
| 総称型パラメータ（`fn id<T>(x)`） | 2 | 他の2backendでは消去されるドキュメント |
| `@` matmul演算子 | 2 | 唯一まだrejectする演算子 |
| 複数モジュールのスクリプト（`import`） | 3 | コンパイラでなく`main.cc`側のreject |

interpとコンパイル済みエンジンの間の差異も3件、未解決のまま残る。
どれもVM以前からのもので、corpusが到達せず、実装の穴というより
言語の問いである: 関数内の`[self] = [5]`はインタプリタでは宣言に
なりコンパイル済み2エンジンではImmutableErrorになる（トップ
レベルでは3レーンとも宣言）。ネイティブbuiltinの`.params`は
インタプリタでは1要素、コンパイル済みでは0要素。`Isolate.spawn`の
ハンドルをinspectすると、コンパイル済みエンジンでだけ内部の
`_core_id`フィールドが見える。
