...
- **optimizerの取り分。** §12.5のIR量は解消した（§12.8）が、`-O2`は
  同じ量のIRに対してAST経路の1.22倍を払っている。残りの1秒は
  「渡した仕事の量」ではなく「optimizerが手間取る形」である。
...
`-O0`ではもうコンパイルが遅くない。残るのは`-O2`の行である。これは
§12.5が特定したのとは別の問題だ — **同じ量のIRに対して**optimizerが
1.36秒、AST経路では0.50秒。しかも基本ブロックは少なく（33,206対43,173）、
alloca も invoke も少なく、関数サイズの Σn^1.9 も小さい。粗い形の指標は
どれもこれを説明しない。だから次の一手は、また数えることではなく
パス単位のプロファイルである。
