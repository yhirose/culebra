...
小さい差異が2件、既知のまま残る。どちらもcorpusが到達せず、VMは
両方でJITと一致している: `(return x).size()` — 発散式を*レシーバ*の
位置に置いた形 — はインタプリタではnilに評価されてメソッド呼びが
TypeErrorになり、コンパイル済み2エンジンはフレームを抜ける。
`(return x)`をそれ以外の位置に置いた形は3エンジンともフレームを
抜ける。古いものも3件残る: 関数内の`[self] = [5]`はインタプリタでは
宣言になりコンパイル済み2エンジンではImmutableErrorになる（トップ
