...
から対応する`OwnedExit`までの直線区間に分岐が一切なく、drop可能な
オブジェクトを登録しうるもの（インスタンス構築、または`drop`を束縛する
プロパティ書き込み、`fixed.inc.h`の`_jit_owned_bind_drop`呼び出し
箇所）も一切ないときに死んでいる。`Call`はこのチェックに特別扱いが
要らない: 呼び先が何を登録しようと、呼び先自身のframeレベルの対が
制御を返す前にそれを排出する（throwパスはframeのmarkだけを解決し
「他のどこでもない」— ネストした対は同期パス限定の最適化であり、
unwindパスからは決して見えない）ので、呼び出し命令を越えて生き残る
ものは何もない。このチェックは単純な逐次スキャンでCFG走査ではない —
中に`if`があるループ本体は最初のラウンドでは保守的なままだが、
不動点反復がその大半を取り戻す: 削除可能だった内側の対が削除されると、
...
レジスタ、所有スタックのmark — をスレッドの`VmStack`に積み、同じループの
...
インストールする: コレクタはモジュール表を取るのと同じように、各
segmentの生きている部分をrootとして取る。`VmStack`に載せられない
