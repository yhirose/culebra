...
- **残っているcleanup。** §12.5のIR量は解消した（§12.8）。`-O2`に残る
  1秒はoptimizerではなく機械語生成である（§12.9）: 最適化後の
  モジュールの1/4がまだunwind cleanupで、AST codegenでは1/10だった。
  この超過を消せば差はそのまま埋まる。そのためにはpadを短くするのでは
  なく**スコープごとに1つ**にする必要があり、それは
  「バイトコードのレジスタの使い回し」ではなく「loweringが自分で持つ
  unwind用スロット」を要求する。
