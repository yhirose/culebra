...
入口2つ — `--jit`と`culebra build` — をバイトコードの上へ移したが、
残されたASTコードジェンをまだ削除していないので出口条件は未達である
...
コミットから）。バッチ3本を終えた時点で、コンパイル型の入口2つ —
`--jit`と`culebra build` — はバイトコードを下ろしており、`jit.h`の
外からそのASTコードジェンに到達する経路は無い。コードジェン自体は
まだ在る: `include/jit.h`は16,446行で、うち80メンバが`peg::Ast`を
取り、今や死んでいる。§7の出口条件 — AST消費者がちょうど1つ — は
したがって未達であり、残りは§12.4である。
...
### 12.4 Phase 3に残っている借り

- **削除。** `jit.h`のASTコードジェンは到達不能だが在る。切り分けは
  機械的 — 署名が`peg::Ast`を取るメンバは死んでいる — で、2つの半分は
  交わらない: 該当は80メンバ、loweringが依存するJITメンバは81個、
  その81個にASTを取るものは1つも無い。（`compile_function_call_raw`と
  その隣人たちは名前に反して値ベースなので残す。`emit_for_open_protocol`が
  呼んでいる。）検証はビルドが通ることより強い: 全テストプログラムの
  `-O0 --emit-llvm`出力が1バイトも変わらないこと。効かない手はブレース
  計数での削除である — このファイルの文字列リテラルとコメントは
  ブレースを含んでおり、それを真に受ける計数器はメンバの末尾を
  行き過ぎてクラスの閉じブレースまで持っていく。
- **載せ替え以降まだ測っていない性能パリティ。** `--jit`は別の
  フロントエンドを通ってLLVMに届くようになった。§10の数字はもう
  一方のものである。
- `--vm-llvm`は今日では`--jit`と同じエンジンであり、レーンというより
  重複したフラグである。ゲートが名指ししているのはこちらなので、
  削除が済むまでは残す。
