...
- README・CLAUDE.md の公開状況・バージョニングに関する記述（pre-1.0 の断り書き、リポジトリの
  公開/非公開の記述など）が現状と食い違っていないか確認する。過去のリリースで一度更新済みなら
  通常は何もしなくてよい — この項目自体、v0.5.0 時点で両ファイルとも既に事実と一致していた
  （かつてここに具体的な旧文言を書いていたが、その文言が直った後も本文だけ古いまま残っていた
  ので、判定基準だけを残す形にした）。
