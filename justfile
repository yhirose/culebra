...
# What a value-type built-in method returns is recorded twice: in the
# reference docs and in CanonSig::return_type, which the compiled lanes read.
# The table stopped being generated, so nothing held the two together.
[group("test")]
[doc("Verify CanonSig::return_type agrees with the reference docs")]
check-canon-return-types:
    tools/checks/check_canon_return_types.sh
...
# The generator and effects transforms re-parse text they built out of a
# body's source, so only the record of each copy leads an error back to where
# it was written. A copy made as a plain string reports the lowering's column.
[group("test")]
[doc("Verify the lowerings copy user source only through anchored MappedSource (ratchet)")]
check-source-anchor-door:
    tools/checks/check_source_anchor_door.sh
