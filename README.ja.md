...
- **標準ライブラリに`import`行が要らない。** `JSON`, `Http`, `Regex`, `PEG`,
  `FS`, `Tensor`, `SQLite`, `Search`をはじめ、すべてプログラム実行前に束縛済みです。
...
culebra側のビルドやインストールも要りません。C++とスクリプトは互いを
呼べるので、ホストの設定や拡張をスクリプトで書けます。
...
- **パイプラインではなくUFCS。** `x.f(...)`は、ユーザー定義型に対する
  自由関数の解決経路も兼ねます。
- **標準ライブラリに`import`は要らない。** 名前空間はプログラム実行前に
