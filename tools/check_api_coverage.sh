...
#
# Usage:
#   tools/check_api_coverage.sh            # gate (fail on a new uncalled name)
#   tools/check_api_coverage.sh --update   # rewrite the allowlist below
set -euo pipefail
cd "$(dirname "$0")/.."

QUICKGUIDE=docs/quick-guide.md
GRAMMAR=include/grammar_def.h
LIST=tools/api_untested.txt
UPDATE=0
[ "${1:-}" = --update ] && UPDATE=1
...
if [ -n "$fixed" ]; then
  ...
  printf '%s\n' "$fixed" | sed 's/^/  /'
  echo "  → run: tools/check_api_coverage.sh --update"
  rc=1
fi
if [ -n "$added" ]; then
