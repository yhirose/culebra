...
# documented names must be the leading params in order, each defaulted exactly
# where the row gives it a default; params the row leaves out must all have
# defaults (Http's `into:`, documented in prose). A function
...
for row in rows {
  ...
  compared += 1
  let actual = ps.map(|p| [p.name, p.has_default])
  let matches = fn (d) {
