...
#
# tools/checks/api_surface.txt (generated by misc/gen_quick_guide.cul) is a
# machine-readable list of every `Ns.fn(...)` the reference docs document,
# native and preamble-defined alike — the thing canon_sigs_table.h alone
# cannot give, since it only knows natives. It is deliberately NOT read off
# docs/quick-guide.md: that file lists only the namespaces a context pack
# carries inline, so reading the population from it would drop half of this
# gate's surface the moment a namespace moved to being named there.
#
# The PEG grammar (include/frontend/grammar_def.h) is the same kind of list for
# keywords, contextual ones included, so a new control-flow form (`cond`,
# `nobreak`, `effect`) shows up here the same way a new namespace function
# does.
#
# A name only the reference docs or the grammar knows about, with no
...
# list (`h.poll()`, `re.test(s)`) — a bare method name cannot be matched
...
# 1. Namespace signatures: one per line, comments aside. Each line that
# starts `Ns.fn(` names a call; receiver methods (`s.trim()`) and idioms
# don't match the anchored pattern and are ignored.
grep -v '^[[:space:]]*#' "$SURFACE" \
...
if [ ! -s "$WORK/sigs.txt" ] || [ ! -s "$WORK/kw.txt" ]; then
  ...
  echo "  is $SURFACE generated, and did the grammar block move?" >&2
  exit 1
fi
