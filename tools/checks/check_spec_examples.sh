...
#
# Usage:
#   tools/checks/check_spec_examples.sh            # gate (fail on a new unpinned section)
#   tools/checks/check_spec_examples.sh --update   # rewrite the list below
#
...
LIST=tools/checks/spec_unpinned_sections.txt
UPDATE=0
[ "${1:-}" = --update ] && UPDATE=1
...
if [ -n "$fixed" ]; then
  ...
  printf '%s\n' "$fixed" | sed 's/^/  /'
  echo "  → run: tools/checks/check_spec_examples.sh --update"
  rc=1
fi
if [ -n "$added" ]; then
