...
- **日本語 docs（`docs/*.ja.md`、`docs/essays/*.ja.md`）は半角英数字（a-z/A-Z/0-9、コードスパンの``` ` ```境界含む）と日本語が直接隣接する箇所に半角スペースを入れない**（`15 年`ではなく`15年`、`culebra の`ではなく`culebraの`、`API を`ではなく`APIを`、`` `ok` フラグ ``ではなく`` `ok`フラグ ``）。以下はそのまま変更しない: ①英単語同士のスペース（`thread / lock / monitor`）②`+`/`/`/`→`等の記号と日本語の間のスペース（`AST + インタプリタ`、`Go / Rust 系`）③コードブロック・インラインコードの中身 ④句点・コロン直後の英語文への遷移（`2009. 494 pages...`）⑤ **見出し行（`#`〜`######`）— GitHub のスラグ生成は見出しテキスト中のスペースをハイフンに変換して `id` にするため、見出しのスペースを消すと目次・相互参照の `#anchor` リンクが壊れる**。見出しを参照する `[text](#anchor)` 形式のリンクテキストも見出しと表記を合わせるため対象外。コード内コメントの日本語文言にも同じ規則を適用してよいが、doctest で実行されるコードブロックは文字列リテラルの意図しない改変を避けるため触らない。機械的に全ファイル適用する際は fenced code block 除外・見出し行除外・アンカーリンクテキスト保護の3点を実装したスクリプトを使い、適用後に `just doctest` と見出しスラグ⇔`#anchor`リンクの突き合わせで検証する。
