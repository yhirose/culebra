...
const auto grammar_ = R"(
  ...
  STATEMENT                <-  STATEMENT_BASE (_sp_ !_nl_ (STMT_MODIFIER_IF / STMT_MODIFIER_UNLESS))?
  STATEMENT_BASE           <-  DEBUGGER / RETURN / THROW / YIELD_FROM / YIELD / BREAK / CONTINUE / DEFER / IMPORT_STMT / EXPORT_STMT / EFFECT_FN_DECL / MULTIFN_DECL / ENUM_DECL / CLASS_DECL / TRAIT_DECL / LEXICAL_SCOPE / EXPRESSION
  # Ruby-style trailing statement modifier: `stmt if cond` / `stmt unless
  # cond`. Parses as STATEMENT = [STATEMENT_BASE, (STMT_MODIFIER_IF |
  # STMT_MODIFIER_UNLESS)]; desugar_postfix_modifiers (parser.h) rewrites it,
  # after AST optimization, into an ordinary IF node so both backends reuse
  # eval_if / compile_if unchanged. `culebra fmt` never desugars (parse_for_format
  # skips it) and prints the modifier form verbatim as its own one-line shape.
  STMT_MODIFIER_IF         <-  if _sp_ !_nl_ EXPRESSION
  STMT_MODIFIER_UNLESS     <-  unless _sp_ !_nl_ EXPRESSION
  ...
  METHOD                   <-  MEMBER_MOD _ IDENTIFIER _ (TYPE_ANNOTATION (_ '=' _ EXPRESSION)? / '=' _ EXPRESSION / PARAMETERS (_ RETURN_TYPE)? _ BLOCK)
  ...
  DEBUGGER                 <-  debugger
  # `return if cond` / `return unless cond` (no value between `return` and the
  # modifier) is ambiguous with "return the value of this if-expression":
  # IDENTIFIER has no keyword exclusion (see is_keyword in parser.h), so once
  # the IF alternative fails here (no block follows), EXPRESSION falls through
  # PRIMARY's alternatives all the way to IDENTIFIER, which happily matches
  # bare `if`/`unless` as if they were 2-/6-letter variable names — silently
  # misparsing the modifier as RETURN's own value and leaving the condition
  # as unparseable leftover text. `&IF` reuses IF's own grammar (rather than
  # re-approximating "is a block coming") to tell the two shapes apart: only
  # attempt EXPRESSION when a real if-expression matches (`return if cond {
  ...
  # TYPE_NAME = single type, optionally `Enum.Variant`-qualified (the one
  # dot names an enum's variant) and optionally Generic, with an optional
  ...
  TYPE_NAME                <-  [A-Z] [a-zA-Z_0-9]* ( '.' [A-Z] [a-zA-Z_0-9]* )? ( _sp_ '<' _sp_ TYPE_ARG ( _sp_ ',' _sp_ TYPE_ARG )* _sp_ '>' )? '?'?
  ...
  #
  # A spec is a sequence of literal chunks and nested `{expr}` fields, so a
  # width or precision can be computed (`{s:>{w}}`, `{x:.{p}f}`). A field's
  # value is spliced into the spec as decimal text before the spec is
  # applied. The nested `{...}` is consumed by SPEC_ARG, which is what
  # leaves the outer `}` to close INTERP_EXPR.
  ...
  # recognized escapes (\n \r \t \\ \" \{ \xHH \uXXXX \UXXXXXXXX) into their
  # byte values. This also lets '\"' and '\{' appear inside the content
  # without prematurely closing the string or starting an interpolation.
  # Neither `\u`/`\U` nor `\x` carries braces, so they need no special
  # grammar case — the generic '\\' . alternative consumes the introducer
  # and the fixed hex digits fall through as ordinary content chars. The
  # decoder validates digit count/range; the grammar just delimits the token.
