...
// table, and sqlite.h's db/stmt tables are all one of these, so a forged or
// stale id behaves the same way everywhere: bounds- and generation-checked,
// never a dereference of arbitrary memory (same posture as the wrap.h foreign
// table and File's fd table; canvas.h's Sprite table is the same slot+
// generation scheme by hand, over a value type rather than T* — not yet on
// this). Header-only and
// value-neutral (no dependency on culebra Value / JitValue / GC) so it can sit
// under any of those files without pulling them into each other.
