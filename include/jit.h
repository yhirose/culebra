...
struct JIT {
 ...
 public:
  ...
  llvm::Function* emit_main_fn(const std::vector<LoadedModule>& modules) {
    ...
    for (const auto& m : modules) {
      infos.push_back(analysis_.analyze_program(*m.ast));
  ...
  void emit_unary_lambda_body(const peg::Ast& lambda_ast,
    ...
    const auto& info = analysis_.func_info.at(&lambda_ast);
 ...
 private:
  ...
  // count.
  llvm::Function* declare_closure_fn(const char* name_prefix, bool has_eh) {
    auto fn = llvm::Function::Create(
        jitFnCalleeType(), llvm::GlobalValue::ExternalLinkage,
        std::format("{}{}", name_prefix, funcCounter_++), module_);
    if (has_eh) fn->setPersonalityFn(get_personality_fn());
  ...
  // Defined in jit_compile_assign.h.
  template <class CompileRhs>
  ...
  Owned compile_match(const peg::Ast& ast) {
    ...
    for (size_t ai = 0; ai < arms.size(); ai++) {
      ...
      // every exit. analysis_.scope_has_defer is populated by
      // scan_eh_defer's MATCH case, so non-block / defer-free arms pay
      // nothing.
      bool body_has_defer = analysis_.scope_has_defer.contains(&body_node);
  ...
  Owned compile_while(const peg::Ast& ast) {
    ...
    bool has_defer = analysis_.scope_has_defer.contains(&body);
    llvm::Value* mark = nullptr;
  ...
  void compile_nobreak_block(const peg::Ast& block, const char* tag) {
    auto fn = builder_.GetInsertBlock()->getParent();
    bool has_defer = analysis_.scope_has_defer.contains(&block);
    llvm::BasicBlock* savedLpad = current_lpad_;
    auto cleanupBB =
  ...
  Owned compile_lexical_scope(const peg::Ast& ast) {
    bool has_defer = analysis_.scope_has_defer.contains(&ast);
  ...
  llvm::Function* emit_constructor_fn(std::string_view class_name,
    ...
    // No personality: CompilerStateSaver nulls current_lpad_ below, so the
    // body emits calls rather than invokes.
    auto fn = declare_closure_fn("__culebra_ctor_", /*has_eh=*/false);
  ...
  // Defined in jit_compile_class.h.
  ...
  Owned compile_import_stmt(const peg::Ast& ast) {
    ...
    if (current_module_path_.empty()) {
      throw culebra::CulebraError(
          "ImportError",
          "`import` is not supported in this context (no module path); "
          "run via `culebra script.cul`",
          ast.line, ast.column);
    }
  ...
  // lookup finds the free-var / captured-local sets. Defined in
  // jit_compile_fn.h.
  ...
  Owned compile_set_mutate_arms(const std::string& method,
    ...
    if (method == "add") {
      ...
      auto biBB2 = llvm::BasicBlock::Create(ctx_, "ma.obj.builtin", fn);
  ...
  Owned compile_function_call_runtime_kwargs(
    ...
    using namespace peg::udl;
    // A malformed arg list aborts before the receiver `self` and any UFCS
    // positional prefix are consumed by the call — release those caller-owned
    // +1s on the throw path. They
  ...
  Owned compile_defer(const peg::Ast& ast) {
    ...
    auto infoIt = analysis_.func_info.find(&ast);
    ...
    auto fn = declare_closure_fn("__culebra_defer_", info.has_eh);
...
inline bool JIT::is_inlinable_lambda(const peg::Ast& ast,
  ...
  auto it = analysis_.func_info.find(&ast);
...
// Compiler-layer fragments: out-of-line definitions of `JIT` members
// declared above, split out for readability. Same contract as the
// runtime-layer fragments at the top of this file: they rely on this
// file's context and are included only from here.
#include <jit_compile_assign.h>
#include <jit_compile_class.h>
#include <jit_compile_fn.h>
