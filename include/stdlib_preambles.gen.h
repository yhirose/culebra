...
let _eff_module = fn() {
  ...
  #
  # A driven tail `resume` re-enters `_drive` recursively, so this stays a plain
  # loop with no per-call abort guard — the guard belongs on the non-recursive
  # `_handle` frame that owns the stack (see `_handle`), keeping deep tail-resume
  # chains shallow. A driven abort clause is still finalized inline here, since
  # it returns rather than unwinding.
