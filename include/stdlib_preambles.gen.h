...
let _peg_module = fn () {
  class Peg {
    ...
    parse(text, path = "") {
      _Peg.parse(self._src, text, self._start, self._optimize, self._packrat, path)
    }
    test(text) {
  ...
  }
  {
    ...
    parse: fn (grammar, text, start = "", optimize = true, packrat = true, path = "") {
      _Peg.parse(grammar, text, start, optimize, packrat, path)
    },
