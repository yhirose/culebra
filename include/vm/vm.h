...
#include <rt/rt.h>  // the shared value model and runtime helpers
#include <base/shared.h>
#include <stdlib/bindings.h>  // culebra_runtime_println + the rt::println decl hook
...
enum class Op : uint8_t {
  ...
  EnumVariant, // bind one variant of enum consts[d], named consts[c], on
               ...
               // variant reaches the namespace back, for `class_of`.
...
enum class BMeth : uint8_t {
  ...
  CausalAttention,
  // layer_norm(gamma, beta): fused layer norm over the last axis, two Tensor
  // params like CausalAttention (its own runtime function).
  LayerNorm,
...
inline std::span<const BMethSpec> bmeth_specs() {
  ...
  static constexpr BMethSpec kSpecs[] = {
      ...
      {"causal_attention", 2, CausalAttention, kRecvTensor, 2, nullptr,
       {Tensor, Tensor}, {"k", "v"}},
      {"layer_norm", 2, LayerNorm, kRecvTensor, 2, nullptr, {Tensor, Tensor},
       {"gamma", "beta"}},
...
// The covering `positions` row, searched. What pos_ix is built from, and what
// chunk_pos_at falls back to before it exists.
inline std::pair<int64_t, int64_t> chunk_pos_search(const Chunk& c, size_t pc) {
  // `positions` is built append-only in emit order, so it is sorted by
  // first_insn — binary-search the covering entry.
  auto it = std::upper_bound(
...
class Compiler {
 ...
 private:
  ...
  struct Binding {
    ...
    // about a binding's OWN unboxed-ness changes on a later write, since
    // every write that could reach here was itself proven eligible for the
    // same class before the binding was ever marked (spec §15.3/§15.4,
    // `project_value_type_spec` memory). nullptr is the overwhelming common
  ...
  ExprResult compile_assignment(const peg::Ast& ast) {
    ...
    // field into `v`'s permanent home — the only copy this mechanism ever
  ...
  ExprResult compile_compound_assign(const peg::Ast& ast,
    ...
    if (b && b->unboxed_layout && base != "??") {
      ...
      ExprResult cur{b->slot, /*owned=*/false, -1, b->unboxed_layout,
                     b->unboxed_class};
      auto r = try_inline_operator(ast, cur, dunder_for_op(cop), {rhs},
                                   {av.rhs});
  ...
  // The same question for UNARY_MINUS's own `__neg__`: exists, is eligible,
  ...
  static bool fold_operand0_is(const peg::Ast& node, std::string_view name) {
    using namespace peg::udl;
    return node.nodes.size() >= 3 && node.nodes[0]->tag == "IDENTIFIER"_ &&
           node.nodes[0]->token == name;
  }
  static bool neg_operand_is(const peg::Ast& node, std::string_view name) {
    using namespace peg::udl;
    return node.nodes.size() >= 2 && node.nodes[1]->tag == "IDENTIFIER"_ &&
  ...
  // identical question `compile_unboxed_value_expr` re-derives when it
  ...
  // no Binding yet for `chain_resolves_to_class`'s `lookup(ast.token)` to
  // find (found live: `v = v + g*DT` always declined until this split —
  // `fold_resolves_to_class`'s own lookahead on operand[0] asked the same
  // question the wrong way, through a binding that could not exist yet).
  // Anything else — a fresh construction of the SAME class, or a chain
  ...
  // emitting half.
  ExprResult compile_unboxed_value_expr(const peg::Ast& rhs) {
    using namespace peg::udl;
    if (rhs.tag == "ADDITIVE"_ || rhs.tag == "MULTIPLICATIVE"_)
  ...
  // The lookahead half of `compile_unboxed_value_expr`: which class this
  // expression resolves to unboxed, dispatched on the same three top-level
  // shapes the emitter handles (fold / negation / construction chain).
  // Asked by a write's classification, by a declaration's own up-front
  // check, and by an operator operand's (`operand_stays_unboxed`).
  const peg::Ast* unboxed_value_expr_class(const peg::Ast& rhs) {
    using namespace peg::udl;
    if (rhs.tag == "ADDITIVE"_ || rhs.tag == "MULTIPLICATIVE"_)
  ...
  // than as a boxed instance — the second allocation a `v = v + C.new(…)`
  // step still pays once the accumulator itself stops allocating. Two
  // things have to hold, and both are cheap to ask:
  //
  //   1. the operand resolves unboxed on its own (`C.new(…)`, `g * DT`,
  //      `-g`, a chain over an already-marked binding), and
  //   2. the dunder's parameter is used only in ways a run can serve.
  //
  ...
  // block's statements would ask this before the class it is about ever
  // registered (found live: an `@value class` declared earlier in the same
  // block as its own `let mut v = C.new(...)` always declined). Interleaving
  ...
  void bind_inline_params(const std::vector<InlineParam>& ps,
    ...
    for (size_t i = 0; i < ps.size(); i++) {
      // An argument that stayed a run (`operand_stays_unboxed` proved the
      // body only reads it as one) binds to N slots instead of one, and
      // the parameter becomes an ordinary unboxed binding for the length
      // of the splice — so `o.x` inside the body compiles through the very
...
struct Exec {
  ...
  static JitValue dispatch(const VmProgram& p, VmFrame*& top) {
    ...
    // binds to the loop, and leaving is an ordinary scope exit that runs
    ...
    static void* const kLabels[] = {
        ...
        &&L_Ge, &&L_ArrayNew, &&L_ArrayAppend, &&L_ArrayPush, &&L_ArrayExtend,
        ...
        &&L_CallKw, &&L_RaiseErr, &&L_Ret, &&L_CellNew, &&L_CellGet,
        &&L_CellSet, &&L_CellRelease, &&L_BindCapture, &&L_ImmutErr,
        &&L_UnboundErr, &&L_MultifnReg, &&L_MfSelf, &&L_ClsSelf, &&L_WkErr,
        &&L_ClassMeta, &&L_DeriveFn, &&L_RegPack, &&L_EnumVariant,
        &&L_TypeMatch, &&L_ClassObj, &&L_BindStatic, &&L_MakeInst,
        &&L_ValueBox, &&L_FieldsInit, &&L_FieldInit, &&L_SelfMerge,
