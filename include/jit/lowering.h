      ...
      switch (in.op) {
        ...
        case Op::PropRaw: {
          ...
          if (in.op == Op::PropRaw) {
            ...
            // of that class. Verified before the value is formed, where the
            // executor verifies it, so the read below sees a constant tag.
            if (in.d != 0)
              view = j.emit_field_type_guard(view, static_cast<int8_t>(in.d),
                                             key);
