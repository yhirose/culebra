...
#
# For every tests/perf/*.cul, run it on the VM executor and
# on the LLVM ORC JIT, then assert speedup (vm / jit) is at least
...
# benches.
#
# Size a new bench so the vm run lands well above 0.1s: `time -p`
...
failed=()
printf "%-22s %10s %10s %10s %10s\n" "benchmark" "vm" "jit" "speedup" "threshold"
printf "%-22s %10s %10s %10s %10s\n" "---------" "------" "---" "-------" "---------"
for f in "${files[@]}"; do
    name=$(basename "$f" .cul)
    threshold=$(awk '
    ...
    t_vm=$({ /usr/bin/time -p "$BIN" --vm "$f" > /dev/null; } 2>&1 | awk '/^real/ {print $2}')
    t_jit=$(   { /usr/bin/time -p "$BIN" --jit "$f" > /dev/null; } 2>&1 | awk '/^real/ {print $2}')
    speedup=$(awk -v i="$t_vm" -v j="$t_jit" 'BEGIN {
        if (j+0 == 0) print "inf"; else printf "%.2f", i/j
    }')
    printf "%-22s %10s %10s %10s %10s\n" "$name" "${t_vm}s" "${t_jit}s" "${speedup}x" "${threshold}x"
