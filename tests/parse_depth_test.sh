...
deep "[" "]" 500 > "$TMP/ok.cul"
run_ok "500-deep array" 1 --vm "$TMP/ok.cul"
...
run_ok "200-deep parens" 1 --vm "$TMP/paren.cul"
...
# frame-slot ceiling (a compile-time "--vm: unsupported: frame larger than
# 8192 slots" reject, both compiled lanes) — a clean, positioned diagnostic,
# which is the language's documented limit for one flat expression since the
# tree-walker (which evaluated it) retired.
{
  printf 'let x = '
